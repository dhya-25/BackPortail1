...
public interface EffectifDAO extends JpaRepository<Effectif, String> {
	...
	@Query(value="SELECT  to_char(date_hist,'yyyy') Annee,t.lib_cat,c.lib_grad, s.lib_serv,p.sexe,count(*)c,\r\n"
			...
			+ "                     group by  to_char(date_hist,'mm/yyyy'),t.lib_cat,c.lib_grad, s.lib_serv,p.sexe,to_char(p.date_hist,'yyyy')",nativeQuery = true)
