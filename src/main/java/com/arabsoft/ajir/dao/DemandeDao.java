package com.arabsoft.ajir.dao;

import java.util.Date;
import java.util.List;

import org.springframework.data.jpa.repository.JpaRepository;
import org.springframework.data.jpa.repository.Query;
import org.springframework.data.repository.query.Param;


import com.arabsoft.ajir.entities.Libre_demande;

public interface DemandeDao extends JpaRepository<Libre_demande, Long> {
	
	@Query(value = "select \r\n"
			+ "       ID_LIBRE_DEMANDE\r\n"
			+ "       COD_SOC,  \r\n"
			+ "       MAT_PERS,\r\n"
			+ "       TYP_DEMANDE,\r\n"
			+ "       DATE_DEMANDE,\r\n"
			+ "       DAT_DEBUT,\r\n"
			+ "       DAT_FIN,\r\n"
			+ "       HEUR_S,\r\n"
			+ "       MIN_S,\r\n"
			+ "       HEUR_R,\r\n"
			+ "       MIN_R,\r\n"
			+ "       COD_TIT,\r\n"
			+ "       COD_TYP,\r\n"
			+ "       COD_THEME,\r\n"
			+ "       COD_GRP_PRET,\r\n"
			+ "       TYP_PRET,\r\n"
			+ "       MNT_DEM,\r\n"
			+ "       NUM_ATTEST,\r\n"
			+ "       TXT_DEM,\r\n"
			+ "       TXT_REPONSE,\r\n"
			+ "       REPONSE_CHEF,\r\n"
			+ "       REPONSE,\r\n"
			+ "       TXT_CHEF,\r\n"
			+ "       FILE_NAME,\r\n"
			+ "       CONTENT_TYPE,\r\n"
			+ "       PATH,\r\n"
			+ "       FICHIER_JOINT ,\r\n"
			+ "       COD_M,\r\n"
			+ "       CNG_TEMPS_FIN,\r\n"
			+ "       CNG_TEMPS_DEBUT,\r\n"
			+ "       CNG_TEMPS,\r\n"
			          +"state,\r\n"
			+ "       COD_AUT,\r\n"
			+ "       ANNEE\r\n"
			+ "from libre_demande\r\n"
			+ "where COD_SOC=:codSoc\r\n" , nativeQuery = true)
	public List<Libre_demande> getListDemande(@Param("codSoc") String codSoc);
	
	
	
	@Query(value="select ID_LIBRE_DEMANDE,COD_SOC,MAT_PERS,TYP_DEMANDE,DATE_DEMANDE,"
			+ "DAT_DEBUT,DAT_FIN,HEUR_S,MIN_S,HEUR_R,MIN_R,COD_TIT,COD_TYP,COD_THEME,COD_GRP_PRET,TYP_PRET,MNT_DEM"
			+ ",NUM_ATTEST,TXT_DEM,TXT_REPONSE,REPONSE_CHEF, REPONSE, TXT_CHEF,FILE_NAME,CONTENT_TYPE"
			+ ",PATH,FICHIER_JOINT,COD_M,CNG_TEMPS_FIN,CNG_TEMPS_DEBUT,CNG_TEMPS,COD_AUT,state"
			+ ",ANNEE from libre_demande where COD_SOC=:codSoc and MAT_PERS=:matpers and TYP_DEMANDE=:type",nativeQuery = true)
	public List<Libre_demande> get(@Param("codSoc")String codSoc,@Param("matpers") String matpers,@Param("type") String type);
	
	
	
	
	

	
	@Query(value="select id_libre_demande,\r\n"
			+ "       l.cod_soc,\r\n"
			+ "       l.mat_pers,\r\n"
			+ "       pk_get_lib.Get_personnel$NomPrenom(l.cod_soc,l.mat_pers) npren_pers,\r\n"
			+ "       typ_demande,\r\n"
			+ "       decode(typ_demande,\r\n"
			+ "              'C',\r\n"
			+ "              'Congés',\r\n"
			+ "              'F',\r\n"
			+ "              'Formation',\r\n"
			+ "              'P',\r\n"
			+ "              'Prets et avances',\r\n"
			+ "              'A',\r\n"
			+ "              'Autorisations',\r\n"
			+ "              'S',\r\n"
			+ "              'Situation Administrtive',\r\n"
			+ "              'D',\r\n"
			+ "              'Attestations','E','Congé exceptionel') lib_demande,\r\n"
			+ "       date_demande,\r\n"
			+ "       dat_debut,\r\n"
			+ "       dat_fin,\r\n"
			+ "       heur_s,\r\n"
			+ "       min_s,\r\n"
			+ "       heur_r,\r\n"
			+ "       min_r,\r\n"
			+ "       cod_tit,\r\n"
			+ "       pk_get_lib.GET_TITRE_FORM$LIB_TIT(WCOD_TIT => l.cod_tit) titre,\r\n"
			+ "       cod_typ,\r\n"
			+ "       pk_get_lib.GET_TYPE_FORMA$LIB_TYP(WCOD_TIT => l.cod_tit,\r\n"
			+ "                                         WCOD_TYP => l.cod_typ) type_formation,\r\n"
			+ "       cod_theme,\r\n"
			+ "       pk_get_lib.GET_THEME$LIB_THEME(WCOD_TIT   => l.cod_tit,\r\n"
			+ "                                      WCOD_TYP   => l.cod_typ,\r\n"
			+ "                                      WCOD_THEME => l.cod_theme) theme,\r\n"
			+ "       cod_grp_pret,\r\n"
			+ "       pk_get_lib.GET_GROUPE_PRE$LIB_GRP_PRET(WCOD_SOC      => l.cod_soc,\r\n"
			+ "                                              WCOD_GRP_PRET => l.cod_grp_pret) group_pret,\r\n"
			+ "       typ_pret,\r\n"
			+ "       pk_get_lib.GET_TYPE_PRET$LIB_PRET(WCOD_SOC      => l.cod_soc,\r\n"
			+ "                                         WCOD_GRP_PRET => l.cod_grp_pret,\r\n"
			+ "                                         WTYP_PRET     => l.typ_pret) lib_pret,\r\n"
			+ "       mnt_dem,\r\n"
			+ "       num_attest,\r\n"
			+ "       txt_dem,\r\n"
			+ "       txt_reponse,\r\n"
			+ "       reponse_chef,\r\n"
			+ "       decode(reponse,'O','Validée','N','Annulée','En instance')reponse,\r\n"
			+ "       txt_chef,\r\n"
			+ "       file_name,\r\n"
			+ "       content_type,\r\n"
			+ "       path,\r\n"
			+"        nat_cng,\r\n"
			+ "       fichier_joint,\r\n"
			+ "       cod_m,\r\n"
			+ "       pk_get_lib.GET_MOTIF_J$LIB_MOT(WCOD_M => l.cod_m) motif,\r\n"
			+ "       cng_temps_fin,\r\n"
			+ "       cng_temps_debut,\r\n"
			+ "       cng_temps,\r\n"
			+ "       cod_aut,\r\n"
			+ "       state,\r\n"
			+ "       pk_get_lib.GET_TYP_AUTORI$LIB_TYP_AUT(l.cod_aut) lib_autorisation,\r\n"
			+ "       annee\r\n"
			+ "  from libre_demande l, personnel p\r\n"
			+ " where l.cod_soc = p.cod_soc \r\n"
			+ "	  and l.mat_pers = p.mat_pers \r\n"
			+ "   and l.cod_soc =:codSoc\r\n"
			+ "   and l.mat_pers =:matpers\r\n" 
			+ "and TYP_DEMANDE=:type",nativeQuery = true)
	public List<Libre_demande> getDem(@Param("codSoc")String codSoc,@Param("matpers") String matpers,@Param("type") String type);

	
	
	@Query(value="select id_libre_demande,\r\n"
			+ "       l.cod_soc,\r\n"
			+ "       l.mat_pers,\r\n"
			+ "       pk_get_lib.Get_personnel$NomPrenom(l.cod_soc,l.mat_pers) npren_pers,\r\n"
			+ "       typ_demande,\r\n"
			+ "       decode(typ_demande,\r\n"
			+ "              'C',\r\n"
			+ "              'Congés',\r\n"
			+ "              'F',\r\n"
			+ "              'Formation',\r\n"
			+ "              'P',\r\n"
			+ "              'Prets et avances',\r\n"
			+ "              'A',\r\n"
			+ "              'Autorisations',\r\n"
			+ "              'S',\r\n"
			+ "              'Situation Administrtive',\r\n"
			+ "              'D',\r\n"
			+ "              'Attestations','E','Congé exceptionel') lib_demande,\r\n"
			+ "       date_demande,\r\n"
			+ "       dat_debut,\r\n"
			+ "       dat_fin,\r\n"
			+ "       heur_s,\r\n"
			+ "       min_s,\r\n"
			+ "       heur_r,\r\n"
			+ "       min_r,\r\n"
			+ " l.state,\r\n"
			+ "       cod_tit,\r\n"
			+ "       pk_get_lib.GET_TITRE_FORM$LIB_TIT(WCOD_TIT => l.cod_tit) titre,\r\n"
			+ "       cod_typ,\r\n"
			+ "       pk_get_lib.GET_TYPE_FORMA$LIB_TYP(WCOD_TIT => l.cod_tit,\r\n"
			+ "                                         WCOD_TYP => l.cod_typ) type_formation,\r\n"
			+ "       cod_theme,\r\n"
			+ "       pk_get_lib.GET_THEME$LIB_THEME(WCOD_TIT   => l.cod_tit,\r\n"
			+ "                                      WCOD_TYP   => l.cod_typ,\r\n"
			+ "                                      WCOD_THEME => l.cod_theme) theme,\r\n"
			+ "       cod_grp_pret,\r\n"
			+ "       pk_get_lib.GET_GROUPE_PRE$LIB_GRP_PRET(WCOD_SOC      => l.cod_soc,\r\n"
			+ "                                              WCOD_GRP_PRET => l.cod_grp_pret) group_pret,\r\n"
			+ "       typ_pret,\r\n"
			+ "       pk_get_lib.GET_TYPE_PRET$LIB_PRET(WCOD_SOC      => l.cod_soc,\r\n"
			+ "                                         WCOD_GRP_PRET => l.cod_grp_pret,\r\n"
			+ "                                         WTYP_PRET     => l.typ_pret) lib_pret,\r\n"
			+ "       mnt_dem,\r\n"
			+ "       num_attest,\r\n"
			+ "       txt_dem,\r\n"
			+ "       txt_reponse,\r\n"
			+ "       reponse_chef,\r\n"
			+ "       reponse,\r\n"
			+ "       txt_chef,\r\n"
			+ "       file_name,\r\n"
			+ "       content_type,\r\n"
			+ "       path,\r\n"
			+ "       fichier_joint,\r\n"
			+ "       cod_m,\r\n"
			+ "       pk_get_lib.GET_MOTIF_J$LIB_MOT(WCOD_M => l.cod_m) motif,\r\n"
			+ "       cng_temps_fin,\r\n"
			+ "       cng_temps_debut,\r\n"
			+ "       cng_temps,\r\n"
			+ "       cod_aut,\r\n"
			+ "       pk_get_lib.GET_TYP_AUTORI$LIB_TYP_AUT(l.cod_aut) lib_autorisation,\r\n"
			+ "       annee\r\n"
			+ "  from libre_demande l, personnel p\r\n"
			+ " where l.cod_soc = p.cod_soc \r\n"
			+ "	  and l.mat_pers = p.mat_pers \r\n"
			+ "   and l.cod_soc =:codSoc\r\n"
			+ "and (l.cod_soc =:codSoc and  l.id_libre_demande in (select id_libre_demande from LIBRE_DEMANDE_VAL where mat_resp =:matpers))\r\n" ,nativeQuery = true)
	public List<Libre_demande> getDemm(@Param("codSoc")String codSoc,@Param("matpers") String matpers);

	
	@Query(value="select id_libre_demande,\r\n"
			+ "       l.cod_soc,\r\n"
			+ "       l.mat_pers,\r\n"
			+ "       (p.nom_pers || ' ' || p.pren_pers) npren_pers,\r\n"
			+ "       typ_demande,\r\n"
			+ "       decode(typ_demande,\r\n"
			+ "              'C',\r\n"
			+ "              'Congés',\r\n"
			+ "              'F',\r\n"
			+ "              'Formation',\r\n"
			+ "              'P',\r\n"
			+ "              'Prets et avances',\r\n"
			+ "              'A',\r\n"
			+ "              'Autorisations',\r\n"
			+ "              'S',\r\n"
			+ "              'Situation Administrtive',\r\n"
			+ "              'D',\r\n"
			+ "              'Attestations','E','Congé exceptionel') lib_demande,\r\n"
			+ "       date_demande,\r\n"
			+ "       dat_debut,\r\n"
			+ "       dat_fin,\r\n"
			+ "       heur_s,\r\n"
			+ "       min_s,\r\n"
			+ "       heur_r,\r\n"
			+ "       nat_cng,\r\n"
			+ "       min_r,\r\n"
			+ "       cod_tit,\r\n"
			+ "       pk_get_lib.GET_TITRE_FORM$LIB_TIT(WCOD_TIT => l.cod_tit) titre,\r\n"
			+ "       cod_typ,\r\n"
			+ "       pk_get_lib.GET_TYPE_FORMA$LIB_TYP(WCOD_TIT => l.cod_tit,\r\n"
			+ "                                         WCOD_TYP => l.cod_typ) type_formation,\r\n"
			+ "       cod_theme,\r\n"
			+ "       pk_get_lib.GET_THEME$LIB_THEME(WCOD_TIT   => l.cod_tit,\r\n"
			+ "                                      WCOD_TYP   => l.cod_typ,\r\n"
			+ "                                      WCOD_THEME => l.cod_theme) theme,\r\n"
			+ "       cod_grp_pret,\r\n"
			+ "       pk_get_lib.GET_GROUPE_PRE$LIB_GRP_PRET(WCOD_SOC      => l.cod_soc,\r\n"
			+ "                                              WCOD_GRP_PRET => l.cod_grp_pret) group_pret,\r\n"
			+ "       typ_pret,\r\n"
			+ "       pk_get_lib.GET_TYPE_PRET$LIB_PRET(WCOD_SOC      => l.cod_soc,\r\n"
			+ "                                         WCOD_GRP_PRET => l.cod_grp_pret,\r\n"
			+ "                                         WTYP_PRET     => l.typ_pret) lib_pret,\r\n"
			+ "       mnt_dem,\r\n"
			+ "       num_attest,\r\n"
			+ "       txt_dem,\r\n"
			+ "       txt_reponse,\r\n"
			+ "       reponse_chef,\r\n"
			+ "       reponse,\r\n"
			+ "       txt_chef,\r\n"
			+ "       file_name,\r\n"
			+ "       content_type,\r\n"
			+ "       path,\r\n"
			+ "       fichier_joint,\r\n"
			+ "       cod_m,\r\n"
			+ "       pk_get_lib.GET_MOTIF_J$LIB_MOT(WCOD_M => l.cod_m) motif,\r\n"
			+ "       cng_temps_fin,\r\n"
			+ "       cng_temps_debut,\r\n"
			+ "       cng_temps,\r\n"
			+ "       cod_aut,\r\n"
			+ "       l.state,\r\n"
			+ "       pk_get_lib.GET_TYP_AUTORI$LIB_TYP_AUT(l.cod_aut) lib_autorisation,\r\n"
			+ "       annee,\r\n"
			+ "       pk_get_lib.GET_SERVICE$LIB_SERV(l.cod_soc,l.cod_serv) lib_serv\r\n"
			+ "  from libre_demande l where l.cod_soc =:codSoc and (l.cod_soc =:codSoc and nvl(l.reponse, 'I') = 'I' and l.id_libre_demande in (select id_libre_demande from LIBRE_DEMANDE_VAL where mat_resp =:matpers)) \r\n"
			+ "    and l.typ_demande not in('P', 'S','D') order by date_demande" ,nativeQuery = true)
	public List<Libre_demande> getDemByRepChefNull(@Param("codSoc")String codSoc,@Param("matpers") String matpers);
	@Query(value="select id_libre_demande,\r\n"
			+ "       l.cod_soc,\r\n"
			+ "       l.mat_pers,\r\n"
			+ "       (p.nom_pers || ' ' || p.pren_pers) npren_pers,\r\n"
			+ "       typ_demande,\r\n"
			+ "       decode(typ_demande,\r\n"
			+ "              'C',\r\n"
			+ "              'Congés',\r\n"
			+ "              'F',\r\n"
			+ "              'Formation',\r\n"
			+ "              'P',\r\n"
			+ "              'Prets et avances',\r\n"
			+ "              'A',\r\n"
			+ "              'Autorisations',\r\n"
			+ "              'S',\r\n"
			+ "              'Situation Administrtive',\r\n"
			+ "              'D',\r\n"
			+ "              'Attestations','E','Congé exceptionel') lib_demande,\r\n"
			+ "       date_demande,\r\n"
			+ "       dat_debut,\r\n"
			+ "       dat_fin,\r\n"
			+ "       heur_s,\r\n"
			+ "       min_s,\r\n"
			+ "       heur_r,\r\n"
			+ "       nat_cng,\r\n"
			+ "       min_r,\r\n"
			+ "       cod_tit,\r\n"
			+ "       pk_get_lib.GET_TITRE_FORM$LIB_TIT(WCOD_TIT => l.cod_tit) titre,\r\n"
			+ "       cod_typ,\r\n"
			+ "       pk_get_lib.GET_TYPE_FORMA$LIB_TYP(WCOD_TIT => l.cod_tit,\r\n"
			+ "                                         WCOD_TYP => l.cod_typ) type_formation,\r\n"
			+ "       cod_theme,\r\n"
			+ "       pk_get_lib.GET_THEME$LIB_THEME(WCOD_TIT   => l.cod_tit,\r\n"
			+ "                                      WCOD_TYP   => l.cod_typ,\r\n"
			+ "                                      WCOD_THEME => l.cod_theme) theme,\r\n"
			+ "       cod_grp_pret,\r\n"
			+ "       pk_get_lib.GET_GROUPE_PRE$LIB_GRP_PRET(WCOD_SOC      => l.cod_soc,\r\n"
			+ "                                              WCOD_GRP_PRET => l.cod_grp_pret) group_pret,\r\n"
			+ "       typ_pret,\r\n"
			+ "       pk_get_lib.GET_TYPE_PRET$LIB_PRET(WCOD_SOC      => l.cod_soc,\r\n"
			+ "                                         WCOD_GRP_PRET => l.cod_grp_pret,\r\n"
			+ "                                         WTYP_PRET     => l.typ_pret) lib_pret,\r\n"
			+ "       mnt_dem,\r\n"
			+ "       num_attest,\r\n"
			+ "       txt_dem,\r\n"
			+ "       txt_reponse,\r\n"
			+ "       reponse_chef,\r\n"
			+ "       reponse,\r\n"
			+ "       txt_chef,\r\n"
			+ "       file_name,\r\n"
			+ "       content_type,\r\n"
			+ "       path,\r\n"
			+ "       fichier_joint,\r\n"
			+ "       cod_m,\r\n"
			+ "       pk_get_lib.GET_MOTIF_J$LIB_MOT(WCOD_M => l.cod_m) motif,\r\n"
			+ "       cng_temps_fin,\r\n"
			+ "       cng_temps_debut,\r\n"
			+ "       cng_temps,\r\n"
			+ "       cod_aut,\r\n"
			+ " l.state,\r\n"
			+ "       pk_get_lib.GET_TYP_AUTORI$LIB_TYP_AUT(l.cod_aut) lib_autorisation,\r\n"
			+ "       annee\r\n"
			+ "  from libre_demande l, personnel p\r\n"
			+ " where l.cod_soc = p.cod_soc \r\n"
			+ "	  and l.mat_pers = p.mat_pers \r\n"
			+ "   and l.cod_soc =:codSoc and (l.cod_soc =:codSoc and  l.id_libre_demande in (select id_libre_demande from LIBRE_DEMANDE_VAL where mat_resp =:matpers)) \r\n"
			+ "   and l.reponse in ('I','N','O') and l.typ_demande not in('P', 'S','D') order by date_demande" ,nativeQuery = true)
	public List<Libre_demande> getDemByRepChefNotNull(@Param("codSoc")String codSoc,@Param("matpers") String matpers);
	
	@Query(value="select id_libre_demande,\r\n"
			+ "			       l.cod_soc,\r\n"
			+ "			       l.mat_pers,\r\n"
			+ "			       pk_get_lib.Get_personnel$NomPrenom(l.cod_soc,l.mat_pers) npren_pers,\r\n"
			+ "			       typ_demande,\r\n"
			+ "			       decode(typ_demande,\r\n"
			+ "			              'C',\r\n"
			+ "			              'Congés',\r\n"
			+ "			              'F',\r\n"
			+ "			              'Formation',\r\n"
			+ "			              'P',\r\n"
			+ "			              'Prets et avances',\r\n"
			+ "			              'A',\r\n"
			+ "			              'Autorisations',\r\n"
			+ "			              'S',\r\n"
			+ "			              'Situation Administrtive',\r\n"
			+ "			              'D',\r\n"
			+ "			              'Attestations','E','Congé exceptionel') lib_demande,\r\n"
			+ "			       date_demande,\r\n"
			+ "			       dat_debut,\r\n"
			+ "			       dat_fin,\r\n"
			+ "			       heur_s,\r\n"
			+ "			       min_s,\r\n"
			+ "			       heur_r,\r\n"
			+ "			       nat_cng,\r\n"
			+ "			       min_r,\r\n"
			+ "			       cod_tit,\r\n"
			+ "			       pk_get_lib.GET_TITRE_FORM$LIB_TIT(WCOD_TIT => l.cod_tit) titre,\r\n"
			+ "			       cod_typ,\r\n"
			+ "			       pk_get_lib.GET_TYPE_FORMA$LIB_TYP(WCOD_TIT => l.cod_tit,\r\n"
			+ "			                                         WCOD_TYP => l.cod_typ) type_formation,\r\n"
			+ "			       cod_theme,\r\n"
			+ "			       pk_get_lib.GET_THEME$LIB_THEME(WCOD_TIT   => l.cod_tit,\r\n"
			+ "			                                      WCOD_TYP   => l.cod_typ,\r\n"
			+ "			                                      WCOD_THEME => l.cod_theme) theme,\r\n"
			+ "			       cod_grp_pret,\r\n"
			+ "			       pk_get_lib.GET_GROUPE_PRE$LIB_GRP_PRET(WCOD_SOC      => l.cod_soc,\r\n"
			+ "			                                              WCOD_GRP_PRET => l.cod_grp_pret) group_pret,\r\n"
			+ "			       typ_pret,\r\n"
			+ "			       pk_get_lib.GET_TYPE_PRET$LIB_PRET(WCOD_SOC      => l.cod_soc,\r\n"
			+ "			                                         WCOD_GRP_PRET => l.cod_grp_pret,\r\n"
			+ "			                                         WTYP_PRET     => l.typ_pret) lib_pret,\r\n"
			+ "			       mnt_dem,\r\n"
			+ "			       num_attest,\r\n"
			+ "			       txt_dem,\r\n"
			+ "			       txt_reponse,\r\n"
			+ "			       reponse_chef,\r\n"
			+ "			       reponse,\r\n"
			+ "			       txt_chef,\r\n"
			+ "			       file_name,\r\n"
			+ "			       content_type,\r\n"
			+ "			       path,\r\n"
			+ "			       fichier_joint,\r\n"
			+ "			       cod_m,\r\n"
			+ "			       pk_get_lib.GET_MOTIF_J$LIB_MOT(WCOD_M => l.cod_m) motif,\r\n"
			+ "			       cng_temps_fin,\r\n"
			+ "			       cng_temps_debut,\r\n"
			+ "			       cng_temps,\r\n"
			+ "			       cod_aut,\r\n"
			+ "			       l.state,\r\n"
			+ "			       pk_get_lib.GET_TYP_AUTORI$LIB_TYP_AUT(l.cod_aut) lib_autorisation,\r\n"
			+ "			       annee,\r\n"
			+ "			       '' lib_serv\r\n"
			+ "			  from libre_demande l\r\n"
			+ "			 where   l.cod_soc =:codSoc and (l.cod_soc =:codSoc and nvl(l.reponse, :reponse) = :reponse and l.id_libre_demande in (select id_libre_demande from LIBRE_DEMANDE_VAL where mat_resp =:matPers)) \r\n"
			+ "			    and l.typ_demande not in('P', 'S','D') \r\n"
			
			+ "                AND l.dat_debut>=:datDebut and l.dat_fin<=:datFin+1 \r\n"
			+ "			order by date_demande" ,nativeQuery = true)
	public List<Libre_demande> getDemByRepChefByDateRep(@Param("codSoc")String codSoc,@Param("matPers") String matpers, @Param("datDebut")Date datDeb,@Param("datFin") Date dateFin,@Param("reponse") String reponse);

	
	@Query(value="select id_libre_demande,\r\n"
			+ "			       l.cod_soc,\r\n"
			+ "			       l.mat_pers,\r\n"
			+ "			       pk_get_lib.Get_personnel$NomPrenom(l.cod_soc,l.mat_pers) npren_pers,\r\n"
			+ "			       typ_demande,\r\n"
			+ "			       decode(typ_demande,\r\n"
			+ "			              'C',\r\n"
			+ "			              'Congés',\r\n"
			+ "			              'F',\r\n"
			+ "			              'Formation',\r\n"
			+ "			              'P',\r\n"
			+ "			              'Prets et avances',\r\n"
			+ "			              'A',\r\n"
			+ "			              'Autorisations',\r\n"
			+ "			              'S',\r\n"
			+ "			              'Situation Administrtive',\r\n"
			+ "			              'D',\r\n"
			+ "			              'Attestations','E','Congé exceptionel') lib_demande,\r\n"
			+ "			       date_demande,\r\n"
			+ "			       dat_debut,\r\n"
			+ "			       dat_fin,\r\n"
			+ "			       heur_s,\r\n"
			+ "			       min_s,\r\n"
			+ "			       heur_r,\r\n"
			+ "			       nat_cng,\r\n"
			+ "			       min_r,\r\n"
			+ "			       cod_tit,\r\n"
			+ "			       pk_get_lib.GET_TITRE_FORM$LIB_TIT(WCOD_TIT => l.cod_tit) titre,\r\n"
			+ "			       cod_typ,\r\n"
			+ "			       pk_get_lib.GET_TYPE_FORMA$LIB_TYP(WCOD_TIT => l.cod_tit,\r\n"
			+ "			                                         WCOD_TYP => l.cod_typ) type_formation,\r\n"
			+ "			       cod_theme,\r\n"
			+ "			       pk_get_lib.GET_THEME$LIB_THEME(WCOD_TIT   => l.cod_tit,\r\n"
			+ "			                                      WCOD_TYP   => l.cod_typ,\r\n"
			+ "			                                      WCOD_THEME => l.cod_theme) theme,\r\n"
			+ "			       cod_grp_pret,\r\n"
			+ "			       pk_get_lib.GET_GROUPE_PRE$LIB_GRP_PRET(WCOD_SOC      => l.cod_soc,\r\n"
			+ "			                                              WCOD_GRP_PRET => l.cod_grp_pret) group_pret,\r\n"
			+ "			       typ_pret,\r\n"
			+ "			       pk_get_lib.GET_TYPE_PRET$LIB_PRET(WCOD_SOC      => l.cod_soc,\r\n"
			+ "			                                         WCOD_GRP_PRET => l.cod_grp_pret,\r\n"
			+ "			                                         WTYP_PRET     => l.typ_pret) lib_pret,\r\n"
			+ "			       mnt_dem,\r\n"
			+ "			       num_attest,\r\n"
			+ "			       txt_dem,\r\n"
			+ "			       txt_reponse,\r\n"
			+ "			       reponse_chef,\r\n"
			+ "			       reponse,\r\n"
			+ "			       txt_chef,\r\n"
			+ "			       file_name,\r\n"
			+ "			       content_type,\r\n"
			+ "			       path,\r\n"
			+ "			       fichier_joint,\r\n"
			+ "			       cod_m,\r\n"
			+ "			       pk_get_lib.GET_MOTIF_J$LIB_MOT(WCOD_M => l.cod_m) motif,\r\n"
			+ "			       cng_temps_fin,\r\n"
			+ "			       cng_temps_debut,\r\n"
			+ "			       cng_temps,\r\n"
			+ "			       cod_aut,\r\n"
			+ "			       l.state,\r\n"
			+ "			       pk_get_lib.GET_TYP_AUTORI$LIB_TYP_AUT(l.cod_aut) lib_autorisation,\r\n"
			+ "			       annee,\r\n"
			+ "			       '' lib_serv\r\n"
			+ "			  from libre_demande l\r\n"
			+ "			 where   l.cod_soc =:codSoc and (l.cod_soc =:codSoc and nvl(l.reponse, :reponse) = :reponse and l.id_libre_demande in (select id_libre_demande from LIBRE_DEMANDE_VAL where mat_resp =:matPers)) \r\n"			
			+ "                AND l.dat_debut>=:datDebut and l.dat_fin<=:datFin+1 \r\n"
			+ "			order by date_demande" ,nativeQuery = true)
	public List<Libre_demande> getDemByRepChefByDateRepPresid(@Param("codSoc")String codSoc,@Param("matPers") String matpers, @Param("datDebut")Date datDeb,@Param("datFin") Date dateFin,@Param("reponse") String reponse);


	@Query(value="select l.id_libre_demande,\r\n"
			+ "                        l.cod_soc,\r\n"
			+ "                        l.mat_pers,\r\n"
			+ "                        pk_get_lib.Get_personnel$NomPrenom(l.cod_soc, l.mat_pers)npren_pers, \r\n"
			+ "                        typ_demande,\r\n"
			+ "                        decode(typ_demande,\r\n"
			+ "                               'C',\r\n"
			+ "                               'Congés',\r\n"
			+ "                               'F',\r\n"
			+ "                               'Formation',\r\n"
			+ "                               'P',\r\n"
			+ "                               'Prets et avances',\r\n"
			+ "                               'A',\r\n"
			+ "                               'Autorisations',\r\n"
			+ "                               'S',\r\n"
			+ "                               'Situation Administrtive',\r\n"
			+ "                               'D',\r\n"
			+ "                               'Attestations','E','Congé exceptionel') lib_demande,\r\n"
			+ "                        date_demande,\r\n"
			+ "                        dat_debut,\r\n"
			+ "                        dat_fin,\r\n"
			+ "                        heur_s,\r\n"
			+ "                        min_s,\r\n"
			+ "                        heur_r,\r\n"
			+ "                        nat_cng,\r\n"
			+ "                        min_r,\r\n"
			+ "                        cod_tit,\r\n"
			+ "                        pk_get_lib.GET_TITRE_FORM$LIB_TIT(WCOD_TIT => l.cod_tit) titre,\r\n"
			+ "                        cod_typ,\r\n"
			+ "                        pk_get_lib.GET_TYPE_FORMA$LIB_TYP(WCOD_TIT => l.cod_tit,\r\n"
			+ "                                                          WCOD_TYP => l.cod_typ) type_formation,\r\n"
			+ "                        cod_theme,\r\n"
			+ "                        pk_get_lib.GET_THEME$LIB_THEME(WCOD_TIT   => l.cod_tit,\r\n"
			+ "                                                       WCOD_TYP   => l.cod_typ,\r\n"
			+ "                                                       WCOD_THEME => l.cod_theme) theme,\r\n"
			+ "                        cod_grp_pret,\r\n"
			+ "                        pk_get_lib.GET_GROUPE_PRE$LIB_GRP_PRET(WCOD_SOC      => l.cod_soc,\r\n"
			+ "                                                               WCOD_GRP_PRET => l.cod_grp_pret) group_pret,\r\n"
			+ "                        typ_pret,\r\n"
			+ "                        pk_get_lib.GET_TYPE_PRET$LIB_PRET(WCOD_SOC      => l.cod_soc,\r\n"
			+ "                                                          WCOD_GRP_PRET => l.cod_grp_pret,\r\n"
			+ "                                                          WTYP_PRET     => l.typ_pret) lib_pret,\r\n"
			+ "                        mnt_dem,\r\n"
			+ "                        num_attest,\r\n"
			+ "                        txt_dem,\r\n"
			+ "                        txt_reponse,\r\n"
			+ "                        reponse_chef,\r\n"
			+ "                        reponse,\r\n"
			+ "                        txt_chef,\r\n"
			+ "                        file_name,\r\n"
			+ "                        content_type,\r\n"
			+ "                        l.path,\r\n"
			+ "                        fichier_joint,\r\n"
			+ "                        cod_m,\r\n"
			+ "                        pk_get_lib.GET_MOTIF_J$LIB_MOT(WCOD_M => l.cod_m) motif,\r\n"
			+ "                        cng_temps_fin,\r\n"
			+ "                        cng_temps_debut,\r\n"
			+ "                        cng_temps,\r\n"
			+ "                        cod_aut,\r\n"
			+ "                        l.state,\r\n"
			+ "                        pk_get_lib.GET_TYP_AUTORI$LIB_TYP_AUT(l.cod_aut) lib_autorisation,\r\n"
			+ "                        annee\r\n"
			+ "                   from libre_demande l,libre_demande_val v      \r\n"
			+ "                  where l.id_libre_demande = v.id_libre_demande                           \r\n"
			+ "                    and l.cod_soc =:codSoc\r\n"
			+ "                    and v.etat_val=:reponse \r\n"
			+ "                    and v.mat_resp=:mat_resp       \r\n"
			+ "                    and nvl('',l.date_demande) between nvl(to_char(:datDebut),l.date_demande) and nvl(to_char(:datFin),l.date_demande)\r\n"
			+ "                 order by date_demande" ,nativeQuery = true)
	public List<Libre_demande> getDemByRepChefByDateRepRH(@Param("codSoc")String codSoc,@Param("mat_resp") String mat_resp,
			@Param("datDebut")Date datDeb,@Param("datFin") Date dateFin,@Param("reponse") String reponse);

	@Query(value="select l.id_libre_demande,\r\n"
			+ "                        l.cod_soc,\r\n"
			+ "                        l.mat_pers,\r\n"
			+ "                        pk_get_lib.Get_personnel$NomPrenom(l.cod_soc, l.mat_pers)npren_pers, \r\n"
			+ "                        typ_demande,\r\n"
			+ "                        decode(typ_demande,\r\n"
			+ "                               'C',\r\n"
			+ "                               'Congés',\r\n"
			+ "                               'F',\r\n"
			+ "                               'Formation',\r\n"
			+ "                               'P',\r\n"
			+ "                               'Prets et avances',\r\n"
			+ "                               'A',\r\n"
			+ "                               'Autorisations',\r\n"
			+ "                               'S',\r\n"
			+ "                               'Situation Administrtive',\r\n"
			+ "                               'D',\r\n"
			+ "                               'Attestations','E','Congé exceptionel') lib_demande,\r\n"
			+ "                        date_demande,\r\n"
			+ "                        dat_debut,\r\n"
			+ "                        dat_fin,\r\n"
			+ "                        heur_s,\r\n"
			+ "                        min_s,\r\n"
			+ "                        heur_r,\r\n"
			+ "                        nat_cng,\r\n"
			+ "                        min_r,\r\n"
			+ "                        cod_tit,\r\n"
			+ "                        pk_get_lib.GET_TITRE_FORM$LIB_TIT(WCOD_TIT => l.cod_tit) titre,\r\n"
			+ "                        cod_typ,\r\n"
			+ "                        pk_get_lib.GET_TYPE_FORMA$LIB_TYP(WCOD_TIT => l.cod_tit,\r\n"
			+ "                                                          WCOD_TYP => l.cod_typ) type_formation,\r\n"
			+ "                        cod_theme,\r\n"
			+ "                        pk_get_lib.GET_THEME$LIB_THEME(WCOD_TIT   => l.cod_tit,\r\n"
			+ "                                                       WCOD_TYP   => l.cod_typ,\r\n"
			+ "                                                       WCOD_THEME => l.cod_theme) theme,\r\n"
			+ "                        cod_grp_pret,\r\n"
			+ "                        pk_get_lib.GET_GROUPE_PRE$LIB_GRP_PRET(WCOD_SOC      => l.cod_soc,\r\n"
			+ "                                                               WCOD_GRP_PRET => l.cod_grp_pret) group_pret,\r\n"
			+ "                        typ_pret,\r\n"
			+ "                        pk_get_lib.GET_TYPE_PRET$LIB_PRET(WCOD_SOC      => l.cod_soc,\r\n"
			+ "                                                          WCOD_GRP_PRET => l.cod_grp_pret,\r\n"
			+ "                                                          WTYP_PRET     => l.typ_pret) lib_pret,\r\n"
			+ "                        mnt_dem,\r\n"
			+ "                        num_attest,\r\n"
			+ "                        txt_dem,\r\n"
			+ "                        txt_reponse,\r\n"
			+ "                        reponse_chef,\r\n"
			+ "                       decode(l.reponse,'I','En instance','O','Validé','N','Refusé') reponse,\r\n"
			+ "                        txt_chef,\r\n"
			+ "                        file_name,\r\n"
			+ "                        content_type,\r\n"
			+ "                        l.path,\r\n"
			+ "                        fichier_joint,\r\n"
			+ "                        cod_m,\r\n"
			+ "                        pk_get_lib.GET_MOTIF_J$LIB_MOT(WCOD_M => l.cod_m) motif,\r\n"
			+ "                        cng_temps_fin,\r\n"
			+ "                        cng_temps_debut,\r\n"
			+ "                        cng_temps,\r\n"
			+ "                        cod_aut,\r\n"
			+ "                        decode(l.state,'I','En instance','O','Validé','N','Refusé') state,\r\n"
			+ "                        pk_get_lib.GET_TYP_AUTORI$LIB_TYP_AUT(l.cod_aut) lib_autorisation,\r\n"
			+ "                        annee\r\n"
			+ "                   from libre_demande l,libre_demande_val v      \r\n"
			+ "                  where l.id_libre_demande = v.id_libre_demande                           \r\n"
			+ "                    and l.cod_soc =:codSoc\r\n"
			+ "                    and v.etat_val=:reponse \r\n"
			
			+ "                    and nvl('',l.date_demande) between nvl(to_char(:datDebut),l.date_demande) and nvl(to_char(:datFin),l.date_demande)\r\n"
			+ "                 order by date_demande" ,nativeQuery = true)
	public List<Libre_demande> getDemRH(@Param("codSoc")String codSoc,
			@Param("datDebut")Date datDeb,@Param("datFin") Date dateFin,@Param("reponse") String reponse);
	@Query(value="select id_libre_demande,\r\n"
			+ "			       l.cod_soc,\r\n"
			+ "			       l.mat_pers,\r\n"
			+ "			       (p.nom_pers || ' ' || p.pren_pers) npren_pers,\r\n"
			+ "			       typ_demande,\r\n"
			+ "			       decode(typ_demande,\r\n"
			+ "			              'C',\r\n"
			+ "			              'Congés',\r\n"
			+ "			              'F',\r\n"
			+ "			              'Formation',\r\n"
			+ "			              'P',\r\n"
			+ "			              'Prets et avances',\r\n"
			+ "			              'A',\r\n"
			+ "			              'Autorisations',\r\n"
			+ "			              'S',\r\n"
			+ "			              'Situation Administrtive',\r\n"
			+ "			              'D',\r\n"
			+ "			              'Attestations','E','Congé exceptionel') lib_demande,\r\n"
			+ "			       date_demande,\r\n"
			+ "			       dat_debut,\r\n"
			+ "			       dat_fin,\r\n"
			+ "			       heur_s,\r\n"
			+ "			       min_s,\r\n"
			+ "			       heur_r,\r\n"
			+ "			       nat_cng,\r\n"
			+ "			       min_r,\r\n"
			+ "			       cod_tit,\r\n"
			+ "			       pk_get_lib.GET_TITRE_FORM$LIB_TIT(WCOD_TIT => l.cod_tit) titre,\r\n"
			+ "			       cod_typ,\r\n"
			+ "			       pk_get_lib.GET_TYPE_FORMA$LIB_TYP(WCOD_TIT => l.cod_tit,\r\n"
			+ "			                                         WCOD_TYP => l.cod_typ) type_formation,\r\n"
			+ "			       cod_theme,\r\n"
			+ "			       pk_get_lib.GET_THEME$LIB_THEME(WCOD_TIT   => l.cod_tit,\r\n"
			+ "			                                      WCOD_TYP   => l.cod_typ,\r\n"
			+ "			                                      WCOD_THEME => l.cod_theme) theme,\r\n"
			+ "			       cod_grp_pret,\r\n"
			+ "			       pk_get_lib.GET_GROUPE_PRE$LIB_GRP_PRET(WCOD_SOC      => l.cod_soc,\r\n"
			+ "			                                              WCOD_GRP_PRET => l.cod_grp_pret) group_pret,\r\n"
			+ "			       typ_pret,\r\n"
			+ "			       pk_get_lib.GET_TYPE_PRET$LIB_PRET(WCOD_SOC      => l.cod_soc,\r\n"
			+ "			                                         WCOD_GRP_PRET => l.cod_grp_pret,\r\n"
			+ "			                                         WTYP_PRET     => l.typ_pret) lib_pret,\r\n"
			+ "			       mnt_dem,\r\n"
			+ "			       num_attest,\r\n"
			+ "			       txt_dem,\r\n"
			+ "			       txt_reponse,\r\n"
			+ "			       reponse_chef,\r\n"
			+ "			       reponse,\r\n"
			+ "			       txt_chef,\r\n"
			+ "			       file_name,\r\n"
			+ "			       content_type,\r\n"
			+ "			       path,\r\n"
			+ "			       fichier_joint,\r\n"
			+ "			       cod_m,\r\n"
			+ "			       pk_get_lib.GET_MOTIF_J$LIB_MOT(WCOD_M => l.cod_m) motif,\r\n"
			+ "			       cng_temps_fin,\r\n"
			+ "			       cng_temps_debut,\r\n"
			+ "			       cng_temps,\r\n"
			+ "			       cod_aut,\r\n"
			+ "			       l.state,\r\n"
			+ "			       pk_get_lib.GET_TYP_AUTORI$LIB_TYP_AUT(l.cod_aut) lib_autorisation,\r\n"
			+ "			       annee,\r\n"
			+ "			       s.lib_serv\r\n"
			+ "			  from libre_demande l, personnel p,service s,libre_demande_val v\r\n"			
			+ "			 where l.cod_soc = p.cod_soc\r\n"
			+ "			   and l.mat_pers = p.mat_pers \r\n"
			+ "               and v.id_libre_demande=l.id_libre_demande\r\n"
			+ "               and l.cod_soc=v.cod_soc\r\n"
			+ "			and p.cod_serv=s.cod_serv \r\n"
			+ "			   and l.cod_soc =:codSoc and (l.cod_soc =:codSoc and nvl(v.etat_val, :reponse) = :reponse ) \r\n"
			+ "			    and l.typ_demande not in('P', 'S','D') \r\n"
			
			+ "                AND l.dat_debut>=:datDebut and l.dat_fin<=:datFin+1 \r\n"
			+ "			order by date_demande" ,nativeQuery = true)
	public List<Libre_demande> getDemByRepChefByDateRepRH(@Param("codSoc")String codSoc,@Param("datDebut")Date datDeb,@Param("datFin") Date dateFin,@Param("reponse") String reponse);

	@Query(value="select id_libre_demande,\r\n"
			+ "			       l.cod_soc,\r\n"
			+ "			       l.mat_pers,\r\n"
			+ "			       pk_get_lib.Get_personnel$NomPrenom(l.cod_soc,l.mat_pers) npren_pers,\r\n"
			+ "			       typ_demande,\r\n"
			+ "			       decode(typ_demande,\r\n"
			+ "			              'C',\r\n"
			+ "			              'Congés',\r\n"
			+ "			              'F',\r\n"
			+ "			              'Formation',\r\n"
			+ "			              'P',\r\n"
			+ "			              'Prets et avances',\r\n"
			+ "			              'A',\r\n"
			+ "			              'Autorisations',\r\n"
			+ "			              'S',\r\n"
			+ "			              'Situation Administrtive',\r\n"
			+ "			              'D',\r\n"
			+ "			              'Attestations','E','Congé exceptionel') lib_demande,\r\n"
			+ "			       date_demande,\r\n"
			+ "			       dat_debut,\r\n"
			+ "			       dat_fin,\r\n"
			+ "			       heur_s,\r\n"
			+ "			       min_s,\r\n"
			+ "			       heur_r,\r\n"
			+ "			       nat_cng,\r\n"
			+ "			       min_r,\r\n"
			+ "			       cod_tit,\r\n"
			+ "			       pk_get_lib.GET_TITRE_FORM$LIB_TIT(WCOD_TIT => l.cod_tit) titre,\r\n"
			+ "			       cod_typ,\r\n"
			+ "			       pk_get_lib.GET_TYPE_FORMA$LIB_TYP(WCOD_TIT => l.cod_tit,\r\n"
			+ "			                                         WCOD_TYP => l.cod_typ) type_formation,\r\n"
			+ "			       cod_theme,\r\n"
			+ "			       pk_get_lib.GET_THEME$LIB_THEME(WCOD_TIT   => l.cod_tit,\r\n"
			+ "			                                      WCOD_TYP   => l.cod_typ,\r\n"
			+ "			                                      WCOD_THEME => l.cod_theme) theme,\r\n"
			+ "			       cod_grp_pret,\r\n"
			+ "			       pk_get_lib.GET_GROUPE_PRE$LIB_GRP_PRET(WCOD_SOC      => l.cod_soc,\r\n"
			+ "			                                              WCOD_GRP_PRET => l.cod_grp_pret) group_pret,\r\n"
			+ "			       typ_pret,\r\n"
			+ "			       pk_get_lib.GET_TYPE_PRET$LIB_PRET(WCOD_SOC      => l.cod_soc,\r\n"
			+ "			                                         WCOD_GRP_PRET => l.cod_grp_pret,\r\n"
			+ "			                                         WTYP_PRET     => l.typ_pret) lib_pret,\r\n"
			+ "			       mnt_dem,\r\n"
			+ "			       num_attest,\r\n"
			+ "			       txt_dem,\r\n"
			+ "			       txt_reponse,\r\n"
			+ "			       reponse_chef,\r\n"
			+ "			       reponse,\r\n"
			+ "			       txt_chef,\r\n"
			+ "			       file_name,\r\n"
			+ "			       content_type,\r\n"
			+ "			       path,\r\n"
			+ "			       fichier_joint,\r\n"
			+ "			       cod_m,\r\n"
			+ "			       pk_get_lib.GET_MOTIF_J$LIB_MOT(WCOD_M => l.cod_m) motif,\r\n"
			+ "			       cng_temps_fin,\r\n"
			+ "			       cng_temps_debut,\r\n"
			+ "			       cng_temps,\r\n"
			+ "			       cod_aut,\r\n"
			+ "			       l.state,\r\n"
			+ "			       pk_get_lib.GET_TYP_AUTORI$LIB_TYP_AUT(l.cod_aut) lib_autorisation,\r\n"
			+ "			       annee,\r\n"
			+ "			       '' lib_serv\r\n"
			+ "			  from libre_demande l where l.cod_soc =:codSoc and (l.cod_soc =:codSoc and nvl(l.reponse, :reponse) = :reponse and l.id_libre_demande in (select id_libre_demande from LIBRE_DEMANDE_VAL where mat_resp =:matPers)) \r\n"
			+ "			    and l.typ_demande not in('P', 'S','D') \r\n"
			
			
			+ "			order by date_demande" ,nativeQuery = true)
	public List<Libre_demande> getDemByRepChefByRep(@Param("codSoc")String codSoc,@Param("matPers") String matpers,@Param("reponse") String reponse);
	
	
	
	@Query(value="select id_libre_demande,\r\n"
			+ "			       l.cod_soc,\r\n"
			+ "			       l.mat_pers,\r\n"
			+ "			       pk_get_lib.Get_personnel$NomPrenom(l.cod_soc,l.mat_pers) npren_pers,\r\n"
			+ "			       typ_demande,\r\n"
			+ "			       decode(typ_demande,\r\n"
			+ "			              'C',\r\n"
			+ "			              'Congés',\r\n"
			+ "			              'F',\r\n"
			+ "			              'Formation',\r\n"
			+ "			              'P',\r\n"
			+ "			              'Prets et avances',\r\n"
			+ "			              'A',\r\n"
			+ "			              'Autorisations',\r\n"
			+ "			              'S',\r\n"
			+ "			              'Situation Administrtive',\r\n"
			+ "			              'D',\r\n"
			+ "			              'Attestations','E','Congé exceptionel') lib_demande,\r\n"
			+ "			       date_demande,\r\n"
			+ "			       dat_debut,\r\n"
			+ "			       dat_fin,\r\n"
			+ "			       heur_s,\r\n"
			+ "			       min_s,\r\n"
			+ "			       heur_r,\r\n"
			+ "			       nat_cng,\r\n"
			+ "			       min_r,\r\n"
			+ "			       cod_tit,\r\n"
			+ "			       pk_get_lib.GET_TITRE_FORM$LIB_TIT(WCOD_TIT => l.cod_tit) titre,\r\n"
			+ "			       cod_typ,\r\n"
			+ "			       pk_get_lib.GET_TYPE_FORMA$LIB_TYP(WCOD_TIT => l.cod_tit,\r\n"
			+ "			                                         WCOD_TYP => l.cod_typ) type_formation,\r\n"
			+ "			       cod_theme,\r\n"
			+ "			       pk_get_lib.GET_THEME$LIB_THEME(WCOD_TIT   => l.cod_tit,\r\n"
			+ "			                                      WCOD_TYP   => l.cod_typ,\r\n"
			+ "			                                      WCOD_THEME => l.cod_theme) theme,\r\n"
			+ "			       cod_grp_pret,\r\n"
			+ "			       pk_get_lib.GET_GROUPE_PRE$LIB_GRP_PRET(WCOD_SOC      => l.cod_soc,\r\n"
			+ "			                                              WCOD_GRP_PRET => l.cod_grp_pret) group_pret,\r\n"
			+ "			       typ_pret,\r\n"
			+ "			       pk_get_lib.GET_TYPE_PRET$LIB_PRET(WCOD_SOC      => l.cod_soc,\r\n"
			+ "			                                         WCOD_GRP_PRET => l.cod_grp_pret,\r\n"
			+ "			                                         WTYP_PRET     => l.typ_pret) lib_pret,\r\n"
			+ "			       mnt_dem,\r\n"
			+ "			       num_attest,\r\n"
			+ "			       txt_dem,\r\n"
			+ "			       txt_reponse,\r\n"
			+ "			       reponse_chef,\r\n"
			+ "			       decode(reponse,'O','Validé','N','Refusé','En instance')reponse,\r\n"
			+ "			       txt_chef,\r\n"
			+ "			       file_name,\r\n"
			+ "			       content_type,\r\n"
			+ "			       path,\r\n"
			+ "			       fichier_joint,\r\n"
			+ "			       cod_m,\r\n"
			+ "			       pk_get_lib.GET_MOTIF_J$LIB_MOT(WCOD_M => l.cod_m) motif,\r\n"
			+ "			       cng_temps_fin,\r\n"
			+ "			       cng_temps_debut,\r\n"
			+ "			       cng_temps,\r\n"
			+ "			       cod_aut,\r\n"
			+ "			       l.state,\r\n"
			+ "			       pk_get_lib.GET_TYP_AUTORI$LIB_TYP_AUT(l.cod_aut) lib_autorisation,\r\n"
			+ "			       annee,\r\n"
			+ "			       '' lib_serv\r\n"
			+ "			  from libre_demande l where l.cod_soc =:codSoc and (l.cod_soc =:codSoc and nvl(l.reponse, :reponse) = :reponse"
			+ "        and l.id_libre_demande in (select id_libre_demande from LIBRE_DEMANDE_VAL where mat_resp =:matPers)) \r\n"
			+ "			order by date_demande" ,nativeQuery = true)
	public List<Libre_demande> getDemByRepChefByRepPresid(@Param("codSoc")String codSoc,@Param("matPers") String matpers,@Param("reponse") String reponse);
	
	@Query(value="select l.id_libre_demande,\r\n"
			+ "                   l.cod_soc,\r\n"
			+ "                   l.mat_pers,\r\n"
			+ "                   (p.nom_pers || ' ' || p.pren_pers) npren_pers,\r\n"
			+ "                   typ_demande,\r\n"
			+ "                   decode(typ_demande,\r\n"
			+ "                          'C',\r\n"
			+ "                          'Congés',\r\n"
			+ "                          'F',\r\n"
			+ "                          'Formation',\r\n"
			+ "                          'P',\r\n"
			+ "                          'Prets et avances',\r\n"
			+ "                          'A',\r\n"
			+ "                          'Autorisations',\r\n"
			+ "                          'S',\r\n"
			+ "                          'Situation Administrtive',\r\n"
			+ "                          'D',\r\n"
			+ "                          'Attestations','E','Congé exceptionel') lib_demande,\r\n"
			+ "                   date_demande,\r\n"
			+ "                   dat_debut,\r\n"
			+ "                   dat_fin,\r\n"
			+ "                   heur_s,\r\n"
			+ "                   min_s,\r\n"
			+ "                   heur_r,\r\n"
			+ "                   nat_cng,\r\n"
			+ "                   min_r,\r\n"
			+ "                   cod_tit,\r\n"
			+ "                   pk_get_lib.GET_TITRE_FORM$LIB_TIT(WCOD_TIT => l.cod_tit) titre,\r\n"
			+ "                   cod_typ,\r\n"
			+ "                   pk_get_lib.GET_TYPE_FORMA$LIB_TYP(WCOD_TIT => l.cod_tit,\r\n"
			+ "                                                     WCOD_TYP => l.cod_typ) type_formation,\r\n"
			+ "                   cod_theme,\r\n"
			+ "                   pk_get_lib.GET_THEME$LIB_THEME(WCOD_TIT   => l.cod_tit,\r\n"
			+ "                                                  WCOD_TYP   => l.cod_typ,\r\n"
			+ "                                                  WCOD_THEME => l.cod_theme) theme,\r\n"
			+ "                   cod_grp_pret,\r\n"
			+ "                   pk_get_lib.GET_GROUPE_PRE$LIB_GRP_PRET(WCOD_SOC      => l.cod_soc,\r\n"
			+ "                                                          WCOD_GRP_PRET => l.cod_grp_pret) group_pret,\r\n"
			+ "                   typ_pret,\r\n"
			+ "                   pk_get_lib.GET_TYPE_PRET$LIB_PRET(WCOD_SOC      => l.cod_soc,\r\n"
			+ "                                                     WCOD_GRP_PRET => l.cod_grp_pret,\r\n"
			+ "                                                     WTYP_PRET     => l.typ_pret) lib_pret,\r\n"
			+ "                   mnt_dem,\r\n"
			+ "                   num_attest,\r\n"
			+ "                   txt_dem,\r\n"
			+ "                   txt_reponse,\r\n"
			+ "                   reponse_chef,\r\n"
			+ "                   reponse,\r\n"
			+ "                   txt_chef,\r\n"
			+ "                   file_name,\r\n"
			+ "                   content_type,\r\n"
			+ "                   path,\r\n"
			+ "                   fichier_joint,\r\n"
			+ "                   cod_m,\r\n"
			+ "                   pk_get_lib.GET_MOTIF_J$LIB_MOT(WCOD_M => l.cod_m) motif,\r\n"
			+ "                   cng_temps_fin,\r\n"
			+ "                   cng_temps_debut,\r\n"
			+ "                   cng_temps,\r\n"
			+ "                   cod_aut,\r\n"
			+ "                   l.state,\r\n"
			+ "                   pk_get_lib.GET_TYP_AUTORI$LIB_TYP_AUT(l.cod_aut) lib_autorisation,\r\n"
			+ "                   annee,\r\n"
			+ "                   s.lib_serv\r\n"
			+ "              from libre_demande l, personnel p,service s\r\n"
			+ "             where l.cod_soc = p.cod_soc\r\n"
			+ "               and l.mat_pers = p.mat_pers\r\n"
			+ "                and l.id_libre_demande in (select distinct id_libre_demande from libre_demande_val where etat_val=:reponse)  \r\n"
			+ "            and p.cod_serv=s.cod_serv \r\n"
			+ "               and l.cod_soc =:codSoc \r\n"
			+ "            order by date_demande" ,nativeQuery = true)
	public List<Libre_demande> getDemByRepChefByRepRh(@Param("codSoc")String codSoc,@Param("reponse") String reponse);
	
	@Query(value="select  l.id_libre_demande,\r\n"
			+ "                        l.cod_soc,\r\n"
			+ "                        l.mat_pers,\r\n"
			+ "                        (p.nom_pers || ' ' || p.pren_pers) npren_pers,\r\n"
			+ "                        typ_demande,\r\n"
			+ "                        decode(typ_demande,\r\n"
			+ "                               'C',\r\n"
			+ "                               'Congés',\r\n"
			+ "                               'F',\r\n"
			+ "                               'Formation',\r\n"
			+ "                               'P',\r\n"
			+ "                               'Prets et avances',\r\n"
			+ "                               'A',\r\n"
			+ "                               'Autorisations',\r\n"
			+ "                               'S',\r\n"
			+ "                               'Situation Administrtive',\r\n"
			+ "                               'D',\r\n"
			+ "                               'Attestations','E','Congé exceptionel') lib_demande,\r\n"
			+ "                        date_demande,\r\n"
			+ "                        dat_debut,\r\n"
			+ "                        dat_fin,\r\n"
			+ "                        heur_s,\r\n"
			+ "                        min_s,\r\n"
			+ "                        heur_r,\r\n"
			+ "                        nat_cng,\r\n"
			+ "                        min_r,\r\n"
			+ "                        cod_tit,\r\n"
			+ "                        pk_get_lib.GET_TITRE_FORM$LIB_TIT(WCOD_TIT => l.cod_tit) titre,\r\n"
			+ "                        cod_typ,\r\n"
			+ "                        pk_get_lib.GET_TYPE_FORMA$LIB_TYP(WCOD_TIT => l.cod_tit,\r\n"
			+ "                                                          WCOD_TYP => l.cod_typ) type_formation,\r\n"
			+ "                        cod_theme,\r\n"
			+ "                        pk_get_lib.GET_THEME$LIB_THEME(WCOD_TIT   => l.cod_tit,\r\n"
			+ "                                                       WCOD_TYP   => l.cod_typ,\r\n"
			+ "                                                       WCOD_THEME => l.cod_theme) theme,\r\n"
			+ "                        cod_grp_pret,\r\n"
			+ "                        pk_get_lib.GET_GROUPE_PRE$LIB_GRP_PRET(WCOD_SOC      => l.cod_soc,\r\n"
			+ "                                                               WCOD_GRP_PRET => l.cod_grp_pret) group_pret,\r\n"
			+ "                        typ_pret,\r\n"
			+ "                        pk_get_lib.GET_TYPE_PRET$LIB_PRET(WCOD_SOC      => l.cod_soc,\r\n"
			+ "                                                          WCOD_GRP_PRET => l.cod_grp_pret,\r\n"
			+ "                                                          WTYP_PRET     => l.typ_pret) lib_pret,\r\n"
			+ "                        mnt_dem,\r\n"
			+ "                        num_attest,\r\n"
			+ "                        txt_dem,\r\n"
			+ "                        txt_reponse,\r\n"
			+ "                        reponse_chef,\r\n"
			+ "                        reponse,\r\n"
			+ "                        txt_chef,\r\n"
			+ "                        file_name,\r\n"
			+ "                        content_type,\r\n"
			+ "                        path,\r\n"
			+ "                        fichier_joint,\r\n"
			+ "                        cod_m,\r\n"
			+ "                        pk_get_lib.GET_MOTIF_J$LIB_MOT(WCOD_M => l.cod_m) motif,\r\n"
			+ "                        cng_temps_fin,\r\n"
			+ "                        cng_temps_debut,\r\n"
			+ "                        cng_temps,\r\n"
			+ "                        cod_aut,\r\n"
			+ "                        l.state,\r\n"
			+ "                        pk_get_lib.GET_TYP_AUTORI$LIB_TYP_AUT(l.cod_aut) lib_autorisation,\r\n"
			+ "                        annee,\r\n"
			+ "                        s.lib_serv\r\n"
			+ "                   from libre_demande l, personnel p,service s\r\n"
			+ "                  where l.cod_soc = p.cod_soc\r\n"
			+ "                    and l.mat_pers = p.mat_pers\r\n"
			+ "                    and l.id_libre_demande in (select distinct id_libre_demande from libre_demande_val where etat_val='I')   \r\n"
			+ "                 and p.cod_serv=s.cod_serv \r\n"
			+ "                                       \r\n"
			+ "            order by date_demande" ,nativeQuery = true)
	public List<Libre_demande> getDemRh();
	@Query(value="select id_libre_demande,\r\n"
			+ "       l.cod_soc,\r\n"
			+ "       l.mat_pers,\r\n"
			+ "       (p.nom_pers || ' ' || p.pren_pers) npren_pers,\r\n"
			+ "       typ_demande,\r\n"
			+ "       decode(typ_demande,\r\n"
			+ "              'C',\r\n"
			+ "              'Congés',\r\n"
			+ "              'F',\r\n"
			+ "              'Formation',\r\n"
			+ "              'P',\r\n"
			+ "              'Prets et avances',\r\n"
			+ "              'A',\r\n"
			+ "              'Autorisations',\r\n"
			+ "              'S',\r\n"
			+ "              'Situation Administrtive',\r\n"
			+ "              'D',\r\n"
			+ "              'Attestations','E','Congé exceptionel') lib_demande,\r\n"
			+ " l.state,\r\n"
			+ "       date_demande,\r\n"
			+ "       dat_debut,\r\n"
			+ "       dat_fin,\r\n"
			+ "       heur_s,\r\n"
			+ "       min_s,\r\n"
			+ "       heur_r,\r\n"
			+ "       min_r,\r\n"
			+ "       cod_tit,\r\n"
			+ "       pk_get_lib.GET_TITRE_FORM$LIB_TIT(WCOD_TIT => l.cod_tit) titre,\r\n"
			+ "       cod_typ,\r\n"
			+ "       pk_get_lib.GET_TYPE_FORMA$LIB_TYP(WCOD_TIT => l.cod_tit,\r\n"
			+ "                                         WCOD_TYP => l.cod_typ) type_formation,\r\n"
			+ "       cod_theme,\r\n"
			+ "       pk_get_lib.GET_THEME$LIB_THEME(WCOD_TIT   => l.cod_tit,\r\n"
			+ "                                      WCOD_TYP   => l.cod_typ,\r\n"
			+ "                                      WCOD_THEME => l.cod_theme) theme,\r\n"
			+ "       cod_grp_pret,\r\n"
			+ "       pk_get_lib.GET_GROUPE_PRE$LIB_GRP_PRET(WCOD_SOC      => l.cod_soc,\r\n"
			+ "                                              WCOD_GRP_PRET => l.cod_grp_pret) group_pret,\r\n"
			+ "       typ_pret,\r\n"
			+ "       pk_get_lib.GET_TYPE_PRET$LIB_PRET(WCOD_SOC      => l.cod_soc,\r\n"
			+ "                                         WCOD_GRP_PRET => l.cod_grp_pret,\r\n"
			+ "                                         WTYP_PRET     => l.typ_pret) lib_pret,\r\n"
			+ "       mnt_dem,\r\n"
			+ "       num_attest,\r\n"
			+ "       txt_dem,\r\n"
			+ "       txt_reponse,\r\n"
			+ "       reponse_chef,\r\n"
			+ "       reponse,\r\n"
			+ "       nat_cng,\r\n"
			+ "       txt_chef,\r\n"
			+ "       file_name,\r\n"
			+ "       content_type,\r\n"
			+ "       path,\r\n"
			+ "       fichier_joint,\r\n"
			+ "       cod_m,\r\n"
			+ "       pk_get_lib.GET_MOTIF_J$LIB_MOT(WCOD_M => l.cod_m) motif,\r\n"
			+ "       cng_temps_fin,\r\n"
			+ "       cng_temps_debut,\r\n"
			+ "       cng_temps,\r\n"
			+ "       cod_aut,\r\n"
			+ "       pk_get_lib.GET_TYP_AUTORI$LIB_TYP_AUT(l.cod_aut) lib_autorisation,\r\n"
			+ "       annee\r\n"
			+ "  from libre_demande l, personnel p\r\n"
			+ " where l.cod_soc = p.cod_soc \r\n"
			+ "	  and l.mat_pers = p.mat_pers \r\n"
			+ "and  l.reponse in ('O','N') \r\n" ,nativeQuery = true)
	public List<Libre_demande> getDemRhByRepNotNull();
	//@Query(value="select p.mat_pers from personnel p where"
	//		+ " (get_pere_by_pere(cod_soc,cod_serv,:serCodSer)=:serCodSer or mat_pers=:matPers) \n"
	//		+ "	and :matPers =(select mat_pers from service where cod_serv=:serCodSer)",nativeQuery = true)
	@Query("select a from Libre_demande a where a.idlibredemande=:x ")
	 public Libre_demande gettByid(@Param("x") Long mat);
	
	
	@Query(value="select id_libre_demande,\r\n"
			+ "       l.cod_soc,\r\n"
			+ "       l.mat_pers,\r\n"
			+ "       (p.nom_pers || ' ' || p.pren_pers) npren_pers,\r\n"
			+ "       typ_demande,\r\n"
			+ "       decode(typ_demande,\r\n"
			+ "              'C',\r\n"
			+ "              'Congés',\r\n"
			+ "              'F',\r\n"
			+ "              'Formation',\r\n"
			+ "              'P',\r\n"
			+ "              'Prets et avances',\r\n"
			+ "              'A',\r\n"
			+ "              'Autorisations',\r\n"
			+ "              'S',\r\n"
			+ "              'Situation Administrtive',\r\n"
			+ "              'D',\r\n"
			+ "              'Attestations','E','Congé exceptionel') lib_demande,\r\n"
			+ "       date_demande,\r\n"
			+ "       dat_debut,\r\n"
			+ "       dat_fin,\r\n"
			+ "       heur_s,\r\n"
			+ "       min_s,\r\n"
			+ "       heur_r,\r\n"
			+ "       min_r,\r\n"
			+ "       cod_tit,\r\n"
			+ "       pk_get_lib.GET_TITRE_FORM$LIB_TIT(WCOD_TIT => l.cod_tit) titre,\r\n"
			+ "       cod_typ,\r\n"
			+ "       pk_get_lib.GET_TYPE_FORMA$LIB_TYP(WCOD_TIT => l.cod_tit,\r\n"
			+ "                                         WCOD_TYP => l.cod_typ) type_formation,\r\n"
			+ "       cod_theme,\r\n"
			+ "       pk_get_lib.GET_THEME$LIB_THEME(WCOD_TIT   => l.cod_tit,\r\n"
			+ "                                      WCOD_TYP   => l.cod_typ,\r\n"
			+ "                                      WCOD_THEME => l.cod_theme) theme,\r\n"
			+ "       cod_grp_pret,\r\n"
			+ "       pk_get_lib.GET_GROUPE_PRE$LIB_GRP_PRET(WCOD_SOC      => l.cod_soc,\r\n"
			+ "                                              WCOD_GRP_PRET => l.cod_grp_pret) group_pret,\r\n"
			+ "       typ_pret,\r\n"
			+ "       pk_get_lib.GET_TYPE_PRET$LIB_PRET(WCOD_SOC      => l.cod_soc,\r\n"
			+ "                                         WCOD_GRP_PRET => l.cod_grp_pret,\r\n"
			+ "                                         WTYP_PRET     => l.typ_pret) lib_pret,\r\n"
			+ "       mnt_dem,\r\n"
			+ "       num_attest,\r\n"
			+ "       txt_dem,\r\n"
			+ "       txt_reponse,\r\n"
			+ "       reponse_chef,\r\n"
			+ "       reponse,\r\n"
			+ "       txt_chef,\r\n"
			+ "       file_name,\r\n"
			+ "       content_type,\r\n"
			+ "       nat_cng,\r\n"
			+ "       path,\r\n"
			+ "       fichier_joint,\r\n"
			+ "       cod_m,\r\n"
			+ "       pk_get_lib.GET_MOTIF_J$LIB_MOT(WCOD_M => l.cod_m) motif,\r\n"
			+ "       cng_temps_fin,\r\n"
			+ "       cng_temps_debut,\r\n"
			+ "       cng_temps,\r\n"
			+ "       cod_aut,\r\n"
			+ " l.state,\r\n"
			+ "       pk_get_lib.GET_TYP_AUTORI$LIB_TYP_AUT(l.cod_aut) lib_autorisation,\r\n"
			+ "       annee\r\n"
			+ "  from libre_demande l, personnel p\r\n"
			+ " where l.cod_soc = p.cod_soc \r\n"
			+ "	  and l.mat_pers = p.mat_pers and l.typ_demande='F' \r\n"
			+ "and  l.reponse in ('O','N') \r\n" ,nativeQuery = true)
	public List<Libre_demande> gethisDemFormationRh();
	@Query(value="select id_libre_demande,\r\n"
			+ "       l.cod_soc,\r\n"
			+ "       l.mat_pers,\r\n"
			+ "       (p.nom_pers || ' ' || p.pren_pers) npren_pers,\r\n"
			+ "       typ_demande,\r\n"
			+ "       decode(typ_demande,\r\n"
			+ "              'C',\r\n"
			+ "              'Congés',\r\n"
			+ "              'F',\r\n"
			+ "              'Formation',\r\n"
			+ "              'P',\r\n"
			+ "              'Prets et avances',\r\n"
			+ "              'A',\r\n"
			+ "              'Autorisations',\r\n"
			+ "              'S',\r\n"
			+ "              'Situation Administrtive',\r\n"
			+ "              'D',\r\n"
			+ "              'Attestations','E','Congé exceptionel') lib_demande,\r\n"
			+ "       date_demande,\r\n"
			+ "       dat_debut,\r\n"
			+ "       dat_fin,\r\n"
			+ "       heur_s,\r\n"
			+ "       min_s,\r\n"
			+ "       heur_r,\r\n"
			+ "       min_r,\r\n"
			+ "       cod_tit,\r\n"
			+ "       pk_get_lib.GET_TITRE_FORM$LIB_TIT(WCOD_TIT => l.cod_tit) titre,\r\n"
			+ "       cod_typ,\r\n"
			+ "       pk_get_lib.GET_TYPE_FORMA$LIB_TYP(WCOD_TIT => l.cod_tit,\r\n"
			+ "                                         WCOD_TYP => l.cod_typ) type_formation,\r\n"
			+ "       cod_theme,\r\n"
			+ "       pk_get_lib.GET_THEME$LIB_THEME(WCOD_TIT   => l.cod_tit,\r\n"
			+ "                                      WCOD_TYP   => l.cod_typ,\r\n"
			+ "                                      WCOD_THEME => l.cod_theme) theme,\r\n"
			+ "       cod_grp_pret,\r\n"
			+ "       pk_get_lib.GET_GROUPE_PRE$LIB_GRP_PRET(WCOD_SOC      => l.cod_soc,\r\n"
			+ "                                              WCOD_GRP_PRET => l.cod_grp_pret) group_pret,\r\n"
			+ "       typ_pret,\r\n"
			+ "       pk_get_lib.GET_TYPE_PRET$LIB_PRET(WCOD_SOC      => l.cod_soc,\r\n"
			+ "                                         WCOD_GRP_PRET => l.cod_grp_pret,\r\n"
			+ "                                         WTYP_PRET     => l.typ_pret) lib_pret,\r\n"
			+ "       mnt_dem,\r\n"
			+ "       num_attest,\r\n"
			+ "       txt_dem,\r\n"
			+ "       txt_reponse,\r\n"
			+ "       reponse_chef,\r\n"
			+ "       nat_cng,\r\n"
			+ "       reponse,\r\n"
			+ "       txt_chef,\r\n"
			+ "       file_name,\r\n"
			+ "       content_type,\r\n"
			+ "       path,\r\n"
			+ "       fichier_joint,\r\n"
			+ "       cod_m,\r\n"
			+ "       pk_get_lib.GET_MOTIF_J$LIB_MOT(WCOD_M => l.cod_m) motif,\r\n"
			+ "       cng_temps_fin,\r\n"
			+ "       cng_temps_debut,\r\n"
			+ "       cng_temps,\r\n"
			+ "       cod_aut,\r\n"
			+ " l.state,\r\n"

			+ "       pk_get_lib.GET_TYP_AUTORI$LIB_TYP_AUT(l.cod_aut) lib_autorisation,\r\n"
			+ "       annee\r\n"
			+ "  from libre_demande l, personnel p\r\n"
			+ " where l.cod_soc = p.cod_soc \r\n"
			+ "	  and l.mat_pers = p.mat_pers and l.typ_demande='A' \r\n"
			+ "and  l.reponse in ('O','N') \r\n" ,nativeQuery = true)
	public List<Libre_demande> gethisDemAutorisationRh();
	@Query(value="select id_libre_demande,\r\n"
			+ "       l.cod_soc,\r\n"
			+ "       l.mat_pers,\r\n"
			+ "       (p.nom_pers || ' ' || p.pren_pers) npren_pers,\r\n"
			+ "       typ_demande,\r\n"
			+ "       decode(typ_demande,\r\n"
			+ "              'C',\r\n"
			+ "              'Congés',\r\n"
			+ "              'F',\r\n"
			+ "              'Formation',\r\n"
			+ "              'P',\r\n"
			+ "              'Prets et avances',\r\n"
			+ "              'A',\r\n"
			+ "              'Autorisations',\r\n"
			+ "              'S',\r\n"
			+ "              'Situation Administrtive',\r\n"
			+ "              'D',\r\n"
			+ "              'Attestations','E','Congé exceptionel') lib_demande,\r\n"
			+ "       date_demande,\r\n"
			+ "       dat_debut,\r\n"
			+ "       dat_fin,\r\n"
			+ "       heur_s,\r\n"
			+ "       min_s,\r\n"
			+ "       heur_r,\r\n"
			+ "       min_r,\r\n"
			+ "       cod_tit,\r\n"
			+ "       pk_get_lib.GET_TITRE_FORM$LIB_TIT(WCOD_TIT => l.cod_tit) titre,\r\n"
			+ "       cod_typ,\r\n"
			+ "       pk_get_lib.GET_TYPE_FORMA$LIB_TYP(WCOD_TIT => l.cod_tit,\r\n"
			+ "                                         WCOD_TYP => l.cod_typ) type_formation,\r\n"
			+ "       cod_theme,\r\n"
			+ "       pk_get_lib.GET_THEME$LIB_THEME(WCOD_TIT   => l.cod_tit,\r\n"
			+ "                                      WCOD_TYP   => l.cod_typ,\r\n"
			+ "                                      WCOD_THEME => l.cod_theme) theme,\r\n"
			+ "       cod_grp_pret,\r\n"
			+ "       pk_get_lib.GET_GROUPE_PRE$LIB_GRP_PRET(WCOD_SOC      => l.cod_soc,\r\n"
			+ "                                              WCOD_GRP_PRET => l.cod_grp_pret) group_pret,\r\n"
			+ "       typ_pret,\r\n"
			+ "       pk_get_lib.GET_TYPE_PRET$LIB_PRET(WCOD_SOC      => l.cod_soc,\r\n"
			+ "                                         WCOD_GRP_PRET => l.cod_grp_pret,\r\n"
			+ "                                         WTYP_PRET     => l.typ_pret) lib_pret,\r\n"
			+ "       mnt_dem,\r\n"
			+ "       num_attest,\r\n"
			+ "       txt_dem,\r\n"
			+ "       txt_reponse,\r\n"
			+ "       reponse_chef,\r\n"
			+ "       reponse,\r\n"
			+ "       txt_chef,\r\n"
			+ "       file_name,\r\n"
			+ "       content_type,\r\n"
			+ "       path,\r\n"
			+ "       fichier_joint,\r\n"
			+ "       nat_cng,\r\n"
			+ "       cod_m,\r\n"
			+ "       pk_get_lib.GET_MOTIF_J$LIB_MOT(WCOD_M => l.cod_m) motif,\r\n"
			+ "       cng_temps_fin,\r\n"
			+ "       cng_temps_debut,\r\n"
			+ "       cng_temps,\r\n"
			+ "       cod_aut,\r\n"
			+ " l.state,\r\n"

			+ "       pk_get_lib.GET_TYP_AUTORI$LIB_TYP_AUT(l.cod_aut) lib_autorisation,\r\n"
			+ "       annee\r\n"
			+ "  from libre_demande l, personnel p\r\n"
			+ " where l.cod_soc = p.cod_soc \r\n"
			+ "	  and l.mat_pers = p.mat_pers and l.typ_demande='P' \r\n"
			+ "and  l.reponse in ('N','O') \r\n" ,nativeQuery = true)
	public List<Libre_demande> gethisDemPretavancesRh();
	@Query(value="select id_libre_demande,\r\n"
			+ "       l.cod_soc,\r\n"
			+ "       l.mat_pers,\r\n"
			+ "       (p.nom_pers || ' ' || p.pren_pers) npren_pers,\r\n"
			+ "       typ_demande,\r\n"
			+ "       decode(typ_demande,\r\n"
			+ "              'C',\r\n"
			+ "              'Congés',\r\n"
			+ "              'F',\r\n"
			+ "              'Formation',\r\n"
			+ "              'P',\r\n"
			+ "              'Prets et avances',\r\n"
			+ "              'A',\r\n"
			+ "              'Autorisations',\r\n"
			+ "              'S',\r\n"
			+ "              'Situation Administrtive',\r\n"
			+ "              'D',\r\n"
			+ "              'Attestations','E','Congé exceptionel') lib_demande,\r\n"
			+ "       date_demande,\r\n"
			+ "       dat_debut,\r\n"
			+ "       dat_fin,\r\n"
			+ "       heur_s,\r\n"
			+ "       min_s,\r\n"
			+ "       heur_r,\r\n"
			+ "       min_r,\r\n"
			+ "       cod_tit,\r\n"
			+ "       pk_get_lib.GET_TITRE_FORM$LIB_TIT(WCOD_TIT => l.cod_tit) titre,\r\n"
			+ "       cod_typ,\r\n"
			+ "       pk_get_lib.GET_TYPE_FORMA$LIB_TYP(WCOD_TIT => l.cod_tit,\r\n"
			+ "                                         WCOD_TYP => l.cod_typ) type_formation,\r\n"
			+ "       cod_theme,\r\n"
			+ "       pk_get_lib.GET_THEME$LIB_THEME(WCOD_TIT   => l.cod_tit,\r\n"
			+ "                                      WCOD_TYP   => l.cod_typ,\r\n"
			+ "                                      WCOD_THEME => l.cod_theme) theme,\r\n"
			+ "       cod_grp_pret,\r\n"
			+ "       pk_get_lib.GET_GROUPE_PRE$LIB_GRP_PRET(WCOD_SOC      => l.cod_soc,\r\n"
			+ "                                              WCOD_GRP_PRET => l.cod_grp_pret) group_pret,\r\n"
			+ "       typ_pret,\r\n"
			+ "       pk_get_lib.GET_TYPE_PRET$LIB_PRET(WCOD_SOC      => l.cod_soc,\r\n"
			+ "                                         WCOD_GRP_PRET => l.cod_grp_pret,\r\n"
			+ "                                         WTYP_PRET     => l.typ_pret) lib_pret,\r\n"
			+ "       mnt_dem,\r\n"
			+ "       num_attest,\r\n"
			+ "       txt_dem,\r\n"
			+ "       txt_reponse,\r\n"
			+ "       reponse_chef,\r\n"
			+ "       reponse,\r\n"
			+ "       txt_chef,\r\n"
			+ "       file_name,\r\n"
			+ "       content_type,\r\n"
			+ "       nat_cng,\r\n"
			+ "       path,\r\n"
			+ "       fichier_joint,\r\n"
			+ "       cod_m,\r\n"
			+ "       pk_get_lib.GET_MOTIF_J$LIB_MOT(WCOD_M => l.cod_m) motif,\r\n"
			+ "       cng_temps_fin,\r\n"
			+ "       cng_temps_debut,\r\n"
			+ "       cng_temps,\r\n"
			+ "       cod_aut,\r\n"
			+ " l.state,\r\n"

			+ "       pk_get_lib.GET_TYP_AUTORI$LIB_TYP_AUT(l.cod_aut) lib_autorisation,\r\n"
			+ "       annee\r\n"
			+ "  from libre_demande l, personnel p\r\n"
			+ " where l.cod_soc = p.cod_soc \r\n"
			+ "	  and l.mat_pers = p.mat_pers and l.typ_demande='D' \r\n"
			+ "and  l.reponse in ('N','O') \r\n" ,nativeQuery = true)
	public List<Libre_demande> gethisDemAttestationRh();
	@Query(value="select id_libre_demande,\r\n"
			+ "       l.cod_soc,\r\n"
			+ "       l.mat_pers,\r\n"
			+ "       (p.nom_pers || ' ' || p.pren_pers) npren_pers,\r\n"
			+ "       typ_demande,\r\n"
			+ "       decode(typ_demande,\r\n"
			+ "              'C',\r\n"
			+ "              'Congés',\r\n"
			+ "              'F',\r\n"
			+ "              'Formation',\r\n"
			+ "              'P',\r\n"
			+ "              'Prets et avances',\r\n"
			+ "              'A',\r\n"
			+ "              'Autorisations',\r\n"
			+ "              'S',\r\n"
			+ "              'Situation Administrtive',\r\n"
			+ "              'D',\r\n"
			+ "              'Attestations','E','Congé exceptionel') lib_demande,\r\n"
			+ "       date_demande,\r\n"
			+ "       dat_debut,\r\n"
			+ "       dat_fin,\r\n"
			+ "       heur_s,\r\n"
			+ "       min_s,\r\n"
			+ "       heur_r,\r\n"
			+ "       min_r,\r\n"
			+ "       cod_tit,\r\n"
			+ "       pk_get_lib.GET_TITRE_FORM$LIB_TIT(WCOD_TIT => l.cod_tit) titre,\r\n"
			+ "       cod_typ,\r\n"
			+ "       pk_get_lib.GET_TYPE_FORMA$LIB_TYP(WCOD_TIT => l.cod_tit,\r\n"
			+ "                                         WCOD_TYP => l.cod_typ) type_formation,\r\n"
			+ "       cod_theme,\r\n"
			+ "       pk_get_lib.GET_THEME$LIB_THEME(WCOD_TIT   => l.cod_tit,\r\n"
			+ "                                      WCOD_TYP   => l.cod_typ,\r\n"
			+ "                                      WCOD_THEME => l.cod_theme) theme,\r\n"
			+ "       cod_grp_pret,\r\n"
			+ "       pk_get_lib.GET_GROUPE_PRE$LIB_GRP_PRET(WCOD_SOC      => l.cod_soc,\r\n"
			+ "                                              WCOD_GRP_PRET => l.cod_grp_pret) group_pret,\r\n"
			+ "       typ_pret,\r\n"
			+ "       pk_get_lib.GET_TYPE_PRET$LIB_PRET(WCOD_SOC      => l.cod_soc,\r\n"
			+ "                                         WCOD_GRP_PRET => l.cod_grp_pret,\r\n"
			+ "                                         WTYP_PRET     => l.typ_pret) lib_pret,\r\n"
			+ "       mnt_dem,\r\n"
			+ "       num_attest,\r\n"
			+ "       txt_dem,\r\n"
			+ "       txt_reponse,\r\n"
			+ "       reponse_chef,\r\n"
			+ "       reponse,\r\n"
			+ "       txt_chef,\r\n"
			+ "       file_name,\r\n"
			+ "       content_type,\r\n"
			+ "       nat_cng,\r\n"
			+ "       path,\r\n"
			+ "       fichier_joint,\r\n"
			+ "       cod_m,\r\n"
			+ "       pk_get_lib.GET_MOTIF_J$LIB_MOT(WCOD_M => l.cod_m) motif,\r\n"
			+ "       cng_temps_fin,\r\n"
			+ "       cng_temps_debut,\r\n"
			+ "       cng_temps,\r\n"
			+ "       cod_aut,\r\n"
			+ " l.state,\r\n"

			+ "       pk_get_lib.GET_TYP_AUTORI$LIB_TYP_AUT(l.cod_aut) lib_autorisation,\r\n"
			+ "       annee\r\n"
			+ "  from libre_demande l, personnel p\r\n"
			+ " where l.cod_soc = p.cod_soc \r\n"
			+ "	  and l.mat_pers = p.mat_pers and l.typ_demande in ('C','E') \r\n"
			+ "and  l.reponse in ('N','O') \r\n" ,nativeQuery = true)
	public List<Libre_demande> gethisDemCongeeRh();
	@Query(value="select id_libre_demande,\r\n"
			+ "       l.cod_soc,\r\n"
			+ "       l.mat_pers,\r\n"
			+ "       (p.nom_pers || ' ' || p.pren_pers) npren_pers,\r\n"
			+ "       typ_demande,\r\n"
			+ "       decode(typ_demande,\r\n"
			+ "              'C',\r\n"
			+ "              'Congés',\r\n"
			+ "              'F',\r\n"
			+ "              'Formation',\r\n"
			+ "              'P',\r\n"
			+ "              'Prets et avances',\r\n"
			+ "              'A',\r\n"
			+ "              'Autorisations',\r\n"
			+ "              'S',\r\n"
			+ "              'Situation Administrtive',\r\n"
			+ "              'D',\r\n"
			+ "              'Attestations','E','Congé exceptionel') lib_demande,\r\n"
			+ "       date_demande,\r\n"
			+ "       dat_debut,\r\n"
			+ "       dat_fin,\r\n"
			+ "       heur_s,\r\n"
			+ "       min_s,\r\n"
			+ "       heur_r,\r\n"
			+ "       min_r,\r\n"
			+ "       cod_tit,\r\n"
			+ "       pk_get_lib.GET_TITRE_FORM$LIB_TIT(WCOD_TIT => l.cod_tit) titre,\r\n"
			+ "       cod_typ,\r\n"
			+ "       pk_get_lib.GET_TYPE_FORMA$LIB_TYP(WCOD_TIT => l.cod_tit,\r\n"
			+ "                                         WCOD_TYP => l.cod_typ) type_formation,\r\n"
			+ "       cod_theme,\r\n"
			+ "       pk_get_lib.GET_THEME$LIB_THEME(WCOD_TIT   => l.cod_tit,\r\n"
			+ "                                      WCOD_TYP   => l.cod_typ,\r\n"
			+ "                                      WCOD_THEME => l.cod_theme) theme,\r\n"
			+ "       cod_grp_pret,\r\n"
			+ "       pk_get_lib.GET_GROUPE_PRE$LIB_GRP_PRET(WCOD_SOC      => l.cod_soc,\r\n"
			+ "                                              WCOD_GRP_PRET => l.cod_grp_pret) group_pret,\r\n"
			+ "       typ_pret,\r\n"
			+ "       pk_get_lib.GET_TYPE_PRET$LIB_PRET(WCOD_SOC      => l.cod_soc,\r\n"
			+ "                                         WCOD_GRP_PRET => l.cod_grp_pret,\r\n"
			+ "                                         WTYP_PRET     => l.typ_pret) lib_pret,\r\n"
			+ "       mnt_dem,\r\n"
			+ "       num_attest,\r\n"
			+ "       txt_dem,\r\n"
			+ "       txt_reponse,\r\n"
			+ "       reponse_chef,\r\n"
			+ "       reponse,\r\n"
			+ "       txt_chef,\r\n"
			+ "       nat_cng,\r\n"
			+ "       file_name,\r\n"
			+ "       content_type,\r\n"
			+ "       path,\r\n"
			+ "       fichier_joint,\r\n"
			+ "       cod_m,\r\n"
			+ "       pk_get_lib.GET_MOTIF_J$LIB_MOT(WCOD_M => l.cod_m) motif,\r\n"
			+ "       cng_temps_fin,\r\n"
			+ "       cng_temps_debut,\r\n"
			+ "       cng_temps,\r\n"
			+ "       cod_aut,\r\n"
			+ "       pk_get_lib.GET_TYP_AUTORI$LIB_TYP_AUT(l.cod_aut) lib_autorisation,\r\n"
			+ " l.state,\r\n"

			+ "       annee\r\n"
			+ "  from libre_demande l, personnel p\r\n"
			+ " where l.cod_soc = p.cod_soc \r\n"
			+ "	  and l.mat_pers = p.mat_pers and l.typ_demande='S' \r\n"
			+ "and  l.reponse in ('N','O') \r\n" ,nativeQuery = true)
	public List<Libre_demande> gethisSituationCRh();
	
	
	public Libre_demande findByIdlibredemande(Long id);
	@Query(value="select id_libre_demande,\r\n"
			+ "       l.cod_soc,\r\n"
			+ "       l.mat_pers,\r\n"
			+ "       (p.nom_pers || ' ' || p.pren_pers) npren_pers,\r\n"
			+ "       typ_demande,\r\n"
			+ "       decode(typ_demande,\r\n"
			+ "              'C',\r\n"
			+ "              'Congés',\r\n"
			+ "              'F',\r\n"
			+ "              'Formation',\r\n"
			+ "              'P',\r\n"
			+ "              'Prets et avances',\r\n"
			+ "              'A',\r\n"
			+ "              'Autorisations',\r\n"
			+ "              'S',\r\n"
			+ "              'Situation Administrtive',\r\n"
			+ "              'D',\r\n"
			+ "              'Attestations','E','Congé exceptionel') lib_demande,\r\n"
			+ "       date_demande,\r\n"
			+ "       dat_debut,\r\n"
			+ "       dat_fin,\r\n"
			+ "       heur_s,\r\n"
			+ "       min_s,\r\n"
			+ "       heur_r,\r\n"
			+ "       min_r,\r\n"
			+ "       cod_tit,\r\n"
			+ "       pk_get_lib.GET_TITRE_FORM$LIB_TIT(WCOD_TIT => l.cod_tit) titre,\r\n"
			+ "       cod_typ,\r\n"
			+ "       pk_get_lib.GET_TYPE_FORMA$LIB_TYP(WCOD_TIT => l.cod_tit,\r\n"
			+ "                                         WCOD_TYP => l.cod_typ) type_formation,\r\n"
			+ "       cod_theme,\r\n"
			+ "       pk_get_lib.GET_THEME$LIB_THEME(WCOD_TIT   => l.cod_tit,\r\n"
			+ "                                      WCOD_TYP   => l.cod_typ,\r\n"
			+ "                                      WCOD_THEME => l.cod_theme) theme,\r\n"
			+ "       cod_grp_pret,\r\n"
			+ "       pk_get_lib.GET_GROUPE_PRE$LIB_GRP_PRET(WCOD_SOC      => l.cod_soc,\r\n"
			+ "                                              WCOD_GRP_PRET => l.cod_grp_pret) group_pret,\r\n"
			+ "       typ_pret,\r\n"
			+ "       pk_get_lib.GET_TYPE_PRET$LIB_PRET(WCOD_SOC      => l.cod_soc,\r\n"
			+ "                                         WCOD_GRP_PRET => l.cod_grp_pret,\r\n"
			+ "                                         WTYP_PRET     => l.typ_pret) lib_pret,\r\n"
			+ "       mnt_dem,\r\n"
			+ "       num_attest,\r\n"
			+ "       txt_dem,\r\n"
			+ "       txt_reponse,\r\n"
			+ "       reponse_chef,\r\n"
			+ "       reponse,\r\n"
			+ "       txt_chef,\r\n"
			+ "       file_name,\r\n"
			+ "       content_type,\r\n"
			+ "       path,\r\n"
			+ "       nat_cng,\r\n"
			+ "       fichier_joint,\r\n"
			+ "       cod_m,\r\n"
			+ "       pk_get_lib.GET_MOTIF_J$LIB_MOT(WCOD_M => l.cod_m) motif,\r\n"
			+ "       cng_temps_fin,\r\n"
			+ "       cng_temps_debut,\r\n"
			+ "       cng_temps,\r\n"
			+ "       cod_aut,\r\n"
			+ "       pk_get_lib.GET_TYP_AUTORI$LIB_TYP_AUT(l.cod_aut) lib_autorisation,\r\n"
			+ "       annee\r\n"
			+ "  from libre_demande l, personnel p\r\n"
			+ " where l.cod_soc = p.cod_soc \r\n"
			+ "	  and l.mat_pers = p.mat_pers and l.TYP_DEMANDE in ('C','E') \r\n"
			+ "and(l.reponse in ('O') or l.reponse_chef is null)"
			+ "and l.reponse is  null \r\n" ,nativeQuery = true)
	public List<Libre_demande> getDemCONGE();
	@Query(value="select id_libre_demande,\r\n"
			+ "       l.cod_soc,\r\n"
			+ "       l.mat_pers,\r\n"
			+ "       (p.nom_pers || ' ' || p.pren_pers) npren_pers,\r\n"
			+ "       typ_demande,\r\n"
			+ "       decode(typ_demande,\r\n"
			+ "              'C',\r\n"
			+ "              'Congés',\r\n"
			+ "              'F',\r\n"
			+ "              'Formation',\r\n"
			+ "              'P',\r\n"
			+ "              'Prets et avances',\r\n"
			+ "              'A',\r\n"
			+ "              'Autorisations',\r\n"
			+ "              'S',\r\n"
			+ "              'Situation Administrtive',\r\n"
			+ "              'D',\r\n"
			+ "              'Attestations','E','Congé exceptionel') lib_demande,\r\n"
			+ "       date_demande,\r\n"
			+ "       dat_debut,\r\n"
			+ "       dat_fin,\r\n"
			+ "       heur_s,\r\n"
			+ "       min_s,\r\n"
			+ "       heur_r,\r\n"
			+ "       min_r,\r\n"
			+ "       cod_tit,\r\n"
			+ "       pk_get_lib.GET_TITRE_FORM$LIB_TIT(WCOD_TIT => l.cod_tit) titre,\r\n"
			+ "       cod_typ,\r\n"
			+ "       pk_get_lib.GET_TYPE_FORMA$LIB_TYP(WCOD_TIT => l.cod_tit,\r\n"
			+ "                                         WCOD_TYP => l.cod_typ) type_formation,\r\n"
			+ "       cod_theme,\r\n"
			+ "       pk_get_lib.GET_THEME$LIB_THEME(WCOD_TIT   => l.cod_tit,\r\n"
			+ "                                      WCOD_TYP   => l.cod_typ,\r\n"
			+ "                                      WCOD_THEME => l.cod_theme) theme,\r\n"
			+ "       cod_grp_pret,\r\n"
			+ "       pk_get_lib.GET_GROUPE_PRE$LIB_GRP_PRET(WCOD_SOC      => l.cod_soc,\r\n"
			+ "                                              WCOD_GRP_PRET => l.cod_grp_pret) group_pret,\r\n"
			+ "       typ_pret,\r\n"
			+ "       pk_get_lib.GET_TYPE_PRET$LIB_PRET(WCOD_SOC      => l.cod_soc,\r\n"
			+ "                                         WCOD_GRP_PRET => l.cod_grp_pret,\r\n"
			+ "                                         WTYP_PRET     => l.typ_pret) lib_pret,\r\n"
			+ "       mnt_dem,\r\n"
			+ "       num_attest,\r\n"
			+ "       txt_dem,\r\n"
			+ "       txt_reponse,\r\n"
			+ "       reponse_chef,\r\n"
			+ "       reponse,\r\n"
			+ "       txt_chef,\r\n"
			+ "       file_name,\r\n"
			+ "       nat_cng,\r\n"
			+ "       content_type,\r\n"
			+ "       path,\r\n"
			+ "       fichier_joint,\r\n"
			+ "       cod_m,\r\n"
			+ "       pk_get_lib.GET_MOTIF_J$LIB_MOT(WCOD_M => l.cod_m) motif,\r\n"
			+ "       cng_temps_fin,\r\n"
			+ "       cng_temps_debut,\r\n"
			+ "       cng_temps,\r\n"
			+ " l.state,\r\n"

			+ "       cod_aut,\r\n"
			+ "       pk_get_lib.GET_TYP_AUTORI$LIB_TYP_AUT(l.cod_aut) lib_autorisation,\r\n"
			+ "       annee\r\n"
			+ "  from libre_demande l, personnel p\r\n"
			+ " where l.cod_soc = p.cod_soc \r\n"
			+ "	  and l.mat_pers = p.mat_pers and l.TYP_DEMANDE='A' \r\n"
			+ "and( l.reponse in ('O') or l.reponse_chef is null)"
			+ "and l.reponse is  null \r\n" ,nativeQuery = true)
	public List<Libre_demande> getDemauto();
	
	
	
	@Query(value="select id_libre_demande,\r\n"
			+ "       l.cod_soc,\r\n"
			+ "       l.mat_pers,\r\n"
			+ "       (p.nom_pers || ' ' || p.pren_pers) npren_pers,\r\n"
			+ "       typ_demande,\r\n"
			+ "       decode(typ_demande,\r\n"
			+ "              'C',\r\n"
			+ "              'Congés',\r\n"
			+ "              'F',\r\n"
			+ "              'Formation',\r\n"
			+ "              'P',\r\n"
			+ "              'Prets et avances',\r\n"
			+ "              'A',\r\n"
			+ "              'Autorisations',\r\n"
			+ "              'S',\r\n"
			+ "              'Situation Administrtive',\r\n"
			+ "              'D',\r\n"
			+ "              'Attestations','E','Congé exceptionel') lib_demande,\r\n"
			+ "       date_demande,\r\n"
			+ "       dat_debut,\r\n"
			+ "       dat_fin,\r\n"
			+ "       heur_s,\r\n"
			+ "       min_s,\r\n"
			+ "       heur_r,\r\n"
			+ "       min_r,\r\n"
			+ "       cod_tit,\r\n"
			+ "       pk_get_lib.GET_TITRE_FORM$LIB_TIT(WCOD_TIT => l.cod_tit) titre,\r\n"
			+ "       cod_typ,\r\n"
			+ "       pk_get_lib.GET_TYPE_FORMA$LIB_TYP(WCOD_TIT => l.cod_tit,\r\n"
			+ "                                         WCOD_TYP => l.cod_typ) type_formation,\r\n"
			+ "       cod_theme,\r\n"
			+ "       pk_get_lib.GET_THEME$LIB_THEME(WCOD_TIT   => l.cod_tit,\r\n"
			+ "                                      WCOD_TYP   => l.cod_typ,\r\n"
			+ "                                      WCOD_THEME => l.cod_theme) theme,\r\n"
			+ "       cod_grp_pret,\r\n"
			+ "       pk_get_lib.GET_GROUPE_PRE$LIB_GRP_PRET(WCOD_SOC      => l.cod_soc,\r\n"
			+ "                                              WCOD_GRP_PRET => l.cod_grp_pret) group_pret,\r\n"
			+ "       typ_pret,\r\n"
			+ "       pk_get_lib.GET_TYPE_PRET$LIB_PRET(WCOD_SOC      => l.cod_soc,\r\n"
			+ "                                         WCOD_GRP_PRET => l.cod_grp_pret,\r\n"
			+ "                                         WTYP_PRET     => l.typ_pret) lib_pret,\r\n"
			+ "       mnt_dem,\r\n"
			+ "       num_attest,\r\n"
			+ "       txt_dem,\r\n"
			+ "       txt_reponse,\r\n"
			+ "       reponse_chef,\r\n"
			+ "       reponse,\r\n"
			+ "       txt_chef,\r\n"
			+ "       file_name,\r\n"
			+ "       content_type,\r\n"
			+ "       path,\r\n"
			+ "       fichier_joint,\r\n"
			+ "       cod_m,\r\n"
			+ "       pk_get_lib.GET_MOTIF_J$LIB_MOT(WCOD_M => l.cod_m) motif,\r\n"
			+ "       cng_temps_fin,\r\n"
			+ "       cng_temps_debut,\r\n"
			+ "       cng_temps,\r\n"
			+ "       cod_aut,\r\n"
			+ " l.state,\r\n"
			+ "       nat_cng,\r\n"
			+ "       pk_get_lib.GET_TYP_AUTORI$LIB_TYP_AUT(l.cod_aut) lib_autorisation,\r\n"
			+ "       annee\r\n"
			+ "  from libre_demande l, personnel p\r\n"
			+ " where l.cod_soc = p.cod_soc \r\n"
			+ "	  and l.mat_pers = p.mat_pers and l.TYP_DEMANDE='F' \r\n"
			+ "and( l.reponse in ('O') or l.reponse_chef is null)"
			+ "and l.reponse is  null \r\n" ,nativeQuery = true)
	public List<Libre_demande> getDemFormation();
	@Query(value="select id_libre_demande,\r\n"
			+ "       l.cod_soc,\r\n"
			+ "       l.mat_pers,\r\n"
			+ "       (p.nom_pers || ' ' || p.pren_pers) npren_pers,\r\n"
			+ "       typ_demande,\r\n"
			+ "       decode(typ_demande,\r\n"
			+ "              'C',\r\n"
			+ "              'Congés',\r\n"
			+ "              'F',\r\n"
			+ "              'Formation',\r\n"
			+ "              'P',\r\n"
			+ "              'Prets et avances',\r\n"
			+ "              'A',\r\n"
			+ "              'Autorisations',\r\n"
			+ "              'S',\r\n"
			+ "              'Situation Administrtive',\r\n"
			+ "              'D',\r\n"
			+ "              'Attestations','E','Congé exceptionel') lib_demande,\r\n"
			+ "       date_demande,\r\n"
			+ "       dat_debut,\r\n"
			+ "       dat_fin,\r\n"
			+ "       heur_s,\r\n"
			+ "       min_s,\r\n"
			+ "       heur_r,\r\n"
			+ "       min_r,\r\n"
			+ "       cod_tit,\r\n"
			+ "       pk_get_lib.GET_TITRE_FORM$LIB_TIT(WCOD_TIT => l.cod_tit) titre,\r\n"
			+ "       cod_typ,\r\n"
			+ "       pk_get_lib.GET_TYPE_FORMA$LIB_TYP(WCOD_TIT => l.cod_tit,\r\n"
			+ "                                         WCOD_TYP => l.cod_typ) type_formation,\r\n"
			+ "       cod_theme,\r\n"
			+ "       pk_get_lib.GET_THEME$LIB_THEME(WCOD_TIT   => l.cod_tit,\r\n"
			+ "                                      WCOD_TYP   => l.cod_typ,\r\n"
			+ "                                      WCOD_THEME => l.cod_theme) theme,\r\n"
			+ "       cod_grp_pret,\r\n"
			+ "       pk_get_lib.GET_GROUPE_PRE$LIB_GRP_PRET(WCOD_SOC      => l.cod_soc,\r\n"
			+ "                                              WCOD_GRP_PRET => l.cod_grp_pret) group_pret,\r\n"
			+ "       typ_pret,\r\n"
			+ "       pk_get_lib.GET_TYPE_PRET$LIB_PRET(WCOD_SOC      => l.cod_soc,\r\n"
			+ "                                         WCOD_GRP_PRET => l.cod_grp_pret,\r\n"
			+ "                                         WTYP_PRET     => l.typ_pret) lib_pret,\r\n"
			+ "       mnt_dem,\r\n"
			+ "       num_attest,\r\n"
			+ "       txt_dem,\r\n"
			+ "       txt_reponse,\r\n"
			+ "       reponse_chef,\r\n"
			+ "       reponse,\r\n"
			+ "       txt_chef,\r\n"
			+ "       file_name,\r\n"
			+ "       content_type,\r\n"
			+ "       path,\r\n"
			+ "       fichier_joint,\r\n"
			+ "       cod_m,\r\n"
			+ "       pk_get_lib.GET_MOTIF_J$LIB_MOT(WCOD_M => l.cod_m) motif,\r\n"
			+ "       cng_temps_fin,\r\n"
			+ "       cng_temps_debut,\r\n"
			+ "       cng_temps,\r\n"
			+ "       cod_aut,\r\n"
			+ " l.state,\r\n"
			+ "       nat_cng,\r\n"
			+ "       pk_get_lib.GET_TYP_AUTORI$LIB_TYP_AUT(l.cod_aut) lib_autorisation,\r\n"
			+ "       annee\r\n"
			+ "  from libre_demande l, personnel p\r\n"
			+ " where l.cod_soc = p.cod_soc \r\n"
			+ "	  and l.mat_pers = p.mat_pers and l.TYP_DEMANDE='P' \r\n"
			+ "and( l.reponse in ('O') or l.reponse_chef is null)"
			+ "and l.reponse is  null \r\n" ,nativeQuery = true)
	public List<Libre_demande> getDemPret();
	@Query(value="select id_libre_demande,\r\n"
			+ "       l.cod_soc,\r\n"
			+ "       l.mat_pers,\r\n"
			+ "       (p.nom_pers || ' ' || p.pren_pers) npren_pers,\r\n"
			+ "       typ_demande,\r\n"
			+ "       decode(typ_demande,\r\n"
			+ "              'C',\r\n"
			+ "              'Congés',\r\n"
			+ "              'F',\r\n"
			+ "              'Formation',\r\n"
			+ "              'P',\r\n"
			+ "              'Prets et avances',\r\n"
			+ "              'A',\r\n"
			+ "              'Autorisations',\r\n"
			+ "              'S',\r\n"
			+ "              'Situation Administrtive',\r\n"
			+ "              'D',\r\n"
			+ "              'Attestations','E','Congé exceptionel') lib_demande,\r\n"
			+ "       date_demande,\r\n"
			+ " l.state,\r\n"

			+ "       dat_debut,\r\n"
			+ "       dat_fin,\r\n"
			+ "       heur_s,\r\n"
			+ "       min_s,\r\n"
			+ "       heur_r,\r\n"
			+ "       min_r,\r\n"
			+ "       cod_tit,\r\n"
			+ "       pk_get_lib.GET_TITRE_FORM$LIB_TIT(WCOD_TIT => l.cod_tit) titre,\r\n"
			+ "       cod_typ,\r\n"
			+ "       pk_get_lib.GET_TYPE_FORMA$LIB_TYP(WCOD_TIT => l.cod_tit,\r\n"
			+ "                                         WCOD_TYP => l.cod_typ) type_formation,\r\n"
			+ "       cod_theme,\r\n"
			+ "       pk_get_lib.GET_THEME$LIB_THEME(WCOD_TIT   => l.cod_tit,\r\n"
			+ "                                      WCOD_TYP   => l.cod_typ,\r\n"
			+ "                                      WCOD_THEME => l.cod_theme) theme,\r\n"
			+ "       cod_grp_pret,\r\n"
			+ "       pk_get_lib.GET_GROUPE_PRE$LIB_GRP_PRET(WCOD_SOC      => l.cod_soc,\r\n"
			+ "                                              WCOD_GRP_PRET => l.cod_grp_pret) group_pret,\r\n"
			+ "       typ_pret,\r\n"
			+ "       pk_get_lib.GET_TYPE_PRET$LIB_PRET(WCOD_SOC      => l.cod_soc,\r\n"
			+ "                                         WCOD_GRP_PRET => l.cod_grp_pret,\r\n"
			+ "                                         WTYP_PRET     => l.typ_pret) lib_pret,\r\n"
			+ "       mnt_dem,\r\n"
			+ "       num_attest,\r\n"
			+ "       txt_dem,\r\n"
			+ "       txt_reponse,\r\n"
			+ "       reponse_chef,\r\n"
			+ "       reponse,\r\n"
			+ "       txt_chef,\r\n"
			+ "       file_name,\r\n"
			+ "       content_type,\r\n"
			+ "       path,\r\n"
			+ "       fichier_joint,\r\n"
			+ "       cod_m,\r\n"
			+ "       nat_cng,\r\n"
			+ "       pk_get_lib.GET_MOTIF_J$LIB_MOT(WCOD_M => l.cod_m) motif,\r\n"
			+ "       cng_temps_fin,\r\n"
			+ "       cng_temps_debut,\r\n"
			+ "       cng_temps,\r\n"
			+ "       cod_aut,\r\n"
			+ "       pk_get_lib.GET_TYP_AUTORI$LIB_TYP_AUT(l.cod_aut) lib_autorisation,\r\n"
			+ "       annee\r\n"
			+ "  from libre_demande l, personnel p\r\n"
			+ " where l.cod_soc = p.cod_soc \r\n"
			+ "	  and l.mat_pers = p.mat_pers and l.TYP_DEMANDE='S' \r\n"
			+ "and( l.reponse in ('O') or l.reponse_chef is null)"
			+ "and l.reponse is  null \r\n" ,nativeQuery = true)
	public List<Libre_demande> getDemSituation();
	@Query(value="select id_libre_demande,\r\n"
			+ "       l.cod_soc,\r\n"
			+ "       l.mat_pers,\r\n"
			+ "       (p.nom_pers || ' ' || p.pren_pers) npren_pers,\r\n"
			+ "       typ_demande,\r\n"
			+ "       decode(typ_demande,\r\n"
			+ "              'C',\r\n"
			+ "              'Congés',\r\n"
			+ "              'F',\r\n"
			+ "              'Formation',\r\n"
			+ "              'P',\r\n"
			+ "              'Prets et avances',\r\n"
			+ "              'A',\r\n"
			+ "              'Autorisations',\r\n"
			+ "              'S',\r\n"
			+ "              'Situation Administrtive',\r\n"
			+ "              'D',\r\n"
			+ "              'Attestations','E','Congé exceptionel') lib_demande,\r\n"
			+ "       date_demande,\r\n"
			+ "       dat_debut,\r\n"
			+ "       dat_fin,\r\n"
			+ "       heur_s,\r\n"
			+ "       min_s,\r\n"
			+ "       heur_r,\r\n"
			+ "       min_r,\r\n"
			+ "       cod_tit,\r\n"
			+ "       pk_get_lib.GET_TITRE_FORM$LIB_TIT(WCOD_TIT => l.cod_tit) titre,\r\n"
			+ "       cod_typ,\r\n"
			+ "       pk_get_lib.GET_TYPE_FORMA$LIB_TYP(WCOD_TIT => l.cod_tit,\r\n"
			+ "                                         WCOD_TYP => l.cod_typ) type_formation,\r\n"
			+ "       cod_theme,\r\n"
			+ "       pk_get_lib.GET_THEME$LIB_THEME(WCOD_TIT   => l.cod_tit,\r\n"
			+ "                                      WCOD_TYP   => l.cod_typ,\r\n"
			+ "                                      WCOD_THEME => l.cod_theme) theme,\r\n"
			+ "       cod_grp_pret,\r\n"
			+ "       pk_get_lib.GET_GROUPE_PRE$LIB_GRP_PRET(WCOD_SOC      => l.cod_soc,\r\n"
			+ "                                              WCOD_GRP_PRET => l.cod_grp_pret) group_pret,\r\n"
			+ "       typ_pret,\r\n"
			+ "       pk_get_lib.GET_TYPE_PRET$LIB_PRET(WCOD_SOC      => l.cod_soc,\r\n"
			+ "                                         WCOD_GRP_PRET => l.cod_grp_pret,\r\n"
			+ "                                         WTYP_PRET     => l.typ_pret) lib_pret,\r\n"
			+ "       mnt_dem,\r\n"
			+ "       num_attest,\r\n"
			+ "       txt_dem,\r\n"
			+ "       nat_cng,\r\n"
			+ "       txt_reponse,\r\n"
			+ "       reponse_chef,\r\n"
			+ "       reponse,\r\n"
			+ "       txt_chef,\r\n"
			+ "       file_name,\r\n"
			+ "       content_type,\r\n"
			+ "       path,\r\n"
			+ "       fichier_joint,\r\n"
			+ "       cod_m,\r\n"
			+ "       pk_get_lib.GET_MOTIF_J$LIB_MOT(WCOD_M => l.cod_m) motif,\r\n"
			+ "       cng_temps_fin,\r\n"
			+ "       cng_temps_debut,\r\n"
			+ "       cng_temps,\r\n"
			+ "       cod_aut,\r\n"
			+ " l.state,\r\n"

			+ "       pk_get_lib.GET_TYP_AUTORI$LIB_TYP_AUT(l.cod_aut) lib_autorisation,\r\n"
			+ "       annee\r\n"
			+ "  from libre_demande l, personnel p\r\n"
			+ " where l.cod_soc = p.cod_soc \r\n"
			+ "	  and l.mat_pers = p.mat_pers and l.TYP_DEMANDE='D' \r\n"
			+ "and( l.reponse in ('O') or l.reponse_chef is null)"
			+ "and l.reponse is  null \r\n" ,nativeQuery = true)
	public List<Libre_demande> getDemAttestation();
	@Query(value="select sold_cng \r\n"
			+ "from sold_cng \r\n"
			+ "where cod_soc = :cod\r\n"
			+ "and mat_pers = :mat\r\n"
			+ "and typ_cng='01'\r\n"
			+ "and ANNEE_CNG= (select max(annee_cng) from sold_cng s where s.cod_soc =:cod and s.mat_pers =:mat) \r\n" ,nativeQuery = true)
	public Long getsoldcng(@Param("cod") String cod,@Param("mat") String mat);
}
	
	
	
//	+ "and l.typ_demande not in('P','S','D')\r\n"
	








	






