		...
		public List<AdrPers> getAdrPers(@Param("codSoc") String codSoc,@Param("matPers") String matPers);
		  ...
		  @Query(value="  select s.mat_pers  from personnel p,service s,adr_pers a\r\n"
			...
			@Query(value="select cod_soc,\r\n"
					...
					+ "from adr_pers\r\n"
					+ "where mat_pers = :matPers",nativeQuery = true)
			public List<AdrPers> GetAdrCheff(@Param("matPers") String matPers);
			
			@Query(value="  select * from adr_pers \r\n"
					+ "where (mat_pers in (\r\n"
					+ "select t.mat_resp from PERS_VALIDEUR t\r\n"
					+ "where mat_pers =:mat))\r\n",nativeQuery=true)
			public List<AdrPers> getEmailChefs(@Param("mat") String mat);
}
